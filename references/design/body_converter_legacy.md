...
Codex-Rosetta 的 body 级别转换器提供了在不同 LLM provider SDK 之间进行完整参数转换的功能。与现有的消息级别转换器不同，body 转换器可以处理 SDK 入口函数的完整参数集合，实现真正的 SDK 兼容性。
...
from codex-rosetta.types.anthropic_types import AnthropicBody  # 使用TypedDict fallback
```
