...
class TestGoogleGenAIConverter:
    ...
    def test_from_provider_response(self):
        """Test from_provider with full API response."""
        provider_data = {
            "response_id": "resp-123",
            "candidates": [
                {
        ...
        # Full response with response_id → IRResponse
        assert result["id"] == "resp-123"
        assert result["choices"][0]["message"]["content"][0]["text"] == "Hi"

    def test_from_provider_candidates_only(self):
        """Test from_provider with candidates but no response_id."""
        provider_data = {
            "candidates": [
                {
                    "index": 0,
        ...
        result = self.converter.from_provider(provider_data)
        # No response_id → returns message list
        assert isinstance(result, list)
        assert len(result) == 1
        assert result[0]["content"][0]["text"] == "Hi"

    def test_from_provider_blocked(self):
        """Test from_provider with blocked response."""
        provider_data = {
            "candidates": [],
        ...
        assert isinstance(result, list)
        assert len(result) == 1
        assert "blocked" in result[0]["content"][0]["text"].lower()

    def test_from_provider_pydantic(self):
        ...
        result = self.converter.from_provider(MockResponse())
        assert isinstance(result, list)
        assert result[0]["content"][0]["text"] == "Hi"

    def test_from_provider_invalid_type(self):
        """Test from_provider with invalid type raises ValueError."""
