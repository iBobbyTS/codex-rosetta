...
The omitted `--model` defaults to `gpt-5.6-terra`. Supply `--model` only when
...
uses `gpt-5.6-terra` as the default GPT cell, and third-party aliases use the
...
|---|---|
| `gpt-5.6-terra` (default GPT; Sol fallback once) | direct OpenAI Responses Lite/code-mode baseline |
