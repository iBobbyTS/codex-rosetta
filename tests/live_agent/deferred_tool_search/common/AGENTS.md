...
- `ALL_TOOLS` is a JavaScript global array inside the `exec` runtime. It is not
  ...
  Functions are available, search first with a generic capability query, read
  the selected exact name, and then invoke the selected runtime entry through
  raw `exec`. Otherwise filter `ALL_TOOLS` directly inside `exec`. In either
  case call the match through
