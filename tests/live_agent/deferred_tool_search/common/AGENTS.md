...
- `ALL_TOOLS` is a JavaScript global array inside the `exec` runtime. It is not
  a shell environment variable. If ordinary `tool_search` and `tool_read`
  Functions plus `invoke_deferred_tool` are available, search first with the
  generic query `Rosetta live candidate`, read the selected exact name, and
  invoke that exact name through `invoke_deferred_tool`. Otherwise filter
  `ALL_TOOLS` directly inside `exec` and call the match through
  `tools[entry.name](args)`. Do not launch Node or a shell through
  `exec_command` as a substitute.
- On the direct `ALL_TOOLS` route for an MCP task, include a compact `catalog`
  array in the same `text()` output as the nested result. Build it by selecting
  every `{name, description}` entry whose description contains
  `Rosetta live candidate`, preserving runtime order. It must contain the
  archive-proof, integer-addition, and color-normalization candidates. Emit the
  catalog and nested return value with exactly one argument:
  `text(JSON.stringify({ catalog, result }))`. The `text()` helper does not
  accept multiple output arguments. On the ordinary-Function route, the
  `tool_search` result is the equivalent catalog evidence. This is diagnostic
