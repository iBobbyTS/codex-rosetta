...
- `steer`: `turn/steer` keeps the active turn ID. It must complete without a
  `<turn_aborted>` marker or a `<codex_runtime_notice>` wrapper.
- `interrupt`: `turn/interrupt` cancels the active upstream stream normally.
  ...
  user-role `<codex_runtime_notice>`, followed by the actual user continuation.
...
established 14,336-token cache block. Adding the explicit wrapper and disclaimer
retained the same block in independent fresh-ID controls. The production path
therefore preserves the marker text and intent but sends it as:
