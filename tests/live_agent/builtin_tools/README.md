...
ledger for an OpenAI-identified provider, using `gpt-5.6-terra` as the default
GPT cell with the local-mode catalog. It tests tool exposure, Code
...
- the model metadata generated by local mode, with `gpt-5.6-terra` as the
  default GPT cell and `gpt-5.6-sol` permitted once after a Terra
...
- `04`: call projected `view_image` on a deterministic local PNG fixture and
  return its image output through Code Mode. The model may omit `detail` or use
  any value present in the model-visible schema; the test must not require an
  unsupported hidden value.
...
- `06`: call projected `view_image` on a deterministic four-quadrant PNG and
  verify that the default multimodal third-party model `qwen3.7-plus` identifies
  all four colors and positions. The same model-visible `detail` rule as task
  `04` applies. Gateway Logs must prove the actual upstream.
