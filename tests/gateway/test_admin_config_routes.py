...
def test_put_provider_enabling_rotation_persists_only_provider_rate(tmp_path):
    data = _config_data()
    provider = data["providers"]["openai"]
    provider["auto_rotate_credentials"] = False
    provider["api_keys"][0]["rate_multiplier"] = 0.25
    ...
    body = _provider_put_body(data, "openai")
    config_path = tmp_path / "config.jsonc"
    provider.pop("current_api_key", None)
    data["model_groups"]["OpenAI"]["provider"] = [
        {"provider": "openai", "credential_uuid": _PRIMARY_CREDENTIAL_UUID}
    ]
    data["model_groups"]["OpenAI"].pop("current_provider", None)
    config_path.write_text(json.dumps(data), encoding="utf-8")
    body.update(
        auto_rotate_credentials=True,
    ...
    assert saved["rate_multiplier"] == 0.5
    assert all("rate_multiplier" not in item for item in saved["api_keys"])


def test_put_provider_disabling_rotation_resets_every_credential_rate(tmp_path):
    data = _config_data()
    provider = data["providers"]["openai"]
    provider["rate_multiplier"] = 0.4
    provider["api_keys"][0]["rate_multiplier"] = 9
    provider["api_keys"].append(
        {
            "uuid": _SECONDARY_CREDENTIAL_UUID,
            "id": "secondary",
    ...
    config_path.write_text(json.dumps(data), encoding="utf-8")
    body = _provider_put_body(data, "openai")
    body.update(auto_rotate_credentials=False, rate_multiplier=7)
    body.pop("current_api_key")
    request = _provider_admin_request(config_path, data, "openai", body)
    ...
    assert response.status_code == 200
    saved = json.loads(config_path.read_text(encoding="utf-8"))["providers"]["openai"]
    assert "rate_multiplier" not in saved
    assert [item["rate_multiplier"] for item in saved["api_keys"]] == [1, 1]


def test_put_provider_legacy_auto_rate_falls_back_and_strips_nested_rates(tmp_path):
    data = _config_data()
    provider = data["providers"]["openai"]
    provider["api_keys"][0]["rate_multiplier"] = 0
    config_path = tmp_path / "config.jsonc"
    config_path.write_text(json.dumps(data), encoding="utf-8")
    body = _provider_put_body(data, "openai")
    request = _provider_admin_request(config_path, data, "openai", body)
    ...
    assert response.status_code == 200
    saved = json.loads(config_path.read_text(encoding="utf-8"))["providers"]["openai"]
    assert saved["rate_multiplier"] == 0
...
def test_put_provider_rejects_invalid_provider_rate_without_write(tmp_path, value):
    ...
    assert response.status_code == 400
    assert b"'rate_multiplier' must be a finite number >= 0" in response.body
    assert config_path.read_text(encoding="utf-8") == original
...
def test_put_provider_disables_rotation_and_uses_previous_effective_uuid(tmp_path):
    ...
    saved = json.loads(config_path.read_text(encoding="utf-8"))
    assert saved["providers"]["openai"]["api_keys"] == [
        {**entry, "rate_multiplier": 1} for entry in body["api_keys"]
    ]
...
def test_put_provider_enables_rotation_and_collapses_pairs_at_first_occurrence(
    ...
    data["model_groups"]["OpenAI"]["current_provider"] = {
        "provider": "openai",
        "credential_uuid": _SECONDARY_CREDENTIAL_UUID,
    }
    config_path = tmp_path / "config.jsonc"
...
def test_provider_rename_updates_string_and_uuid_pair_candidates():
    ...
    assert data["model_groups"]["mixed"]["current_provider"] == {
        "provider": "renamed",
        "credential_uuid": _PRIMARY_CREDENTIAL_UUID,
    }
...
def test_put_provider_previews_then_confirms_referenced_credential_deletion(
    ...
    assert saved["model_groups"]["Promote"]["current_provider"] == {
        "provider": "openai",
        "credential_uuid": _SECONDARY_CREDENTIAL_UUID,
    }
    assert saved["model_groups"]["Empty"]["provider"] == []
    assert "current_provider" not in saved["model_groups"]["Empty"]
...
def test_get_config_exposes_only_active_model_group_provider(tmp_path):
    ...
    assert group["current_provider"] == "openai"
...
def test_get_config_fails_closed_when_json_fields_reconstruct_credential(tmp_path):
    config = _config_data()
    config["providers"]["openai"]["api_keys"][0]["key"] = "sk-secret"
    runtime_config = GatewayConfig(config)
    runtime_config.model_group_rings["OpenAI"].mark_failed(
        "openai", '{"parts":["sk-","secret"]}'
    )
    config_path = tmp_path / "config.jsonc"
    config_path.write_text(json.dumps(config), encoding="utf-8")
...
def test_get_config_fails_closed_when_sse_string_scalars_reconstruct_credential(
    tmp_path,
):
    config = _config_data()
    config["providers"]["openai"]["api_keys"][0]["key"] = "sk-secret"
...
def test_put_and_get_model_group_preserve_order_with_non_first_current(tmp_path):
    config = _config_data()
    config["providers"]["secondary"] = {
        **config["providers"]["openai"],
        "base_urls": ["https://secondary.example.com"],
    ...
    assert saved["model_groups"]["OpenAI"]["current_provider"] == "secondary"
    assert app.gateway_config.model_group_rings["OpenAI"].current == "secondary"

    projected = json.loads(_run(get_config(SimpleNamespace(app=app))).body)
    group = projected["model_groups"]["OpenAI"]
    assert group["provider"] == "secondary"
    assert group["current_provider"] == "secondary"
    assert [row["name"] for row in group["providers"]] == ["openai", "secondary"]
    assert [row["current"] for row in group["providers"]] == [False, True]


def test_put_model_group_rejects_nonmember_current_without_write(tmp_path):
    config = _config_data()
    original = json.dumps(config)
    config_path = tmp_path / "config.jsonc"
    config_path.write_text(original, encoding="utf-8")
    ...
    assert response.status_code == 400
    assert b"current_provider" in response.body
    assert config_path.read_text(encoding="utf-8") == original
    assert request.app.gateway_config is initial_config
