...
OPENAI_ENV = {"OPENAI_BASE_URL": BASE_URL, "OPENAI_API_KEY": API_KEY, "CODEX_PROVIDER": "openai"}
ANTHROPIC_ENV = {"ANTHROPIC_BASE_URL": BASE_URL.replace("/v1", ""), "ANTHROPIC_API_KEY": API_KEY}
...
for agent, env in AGENTS:
    for model in MODELS:
        ...
        try:
            result = run_sync(
                prompt, agent=agent, model=model, env=env,
                max_turns=1, timeout=90,
            )
