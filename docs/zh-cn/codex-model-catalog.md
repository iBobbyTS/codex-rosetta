...
这些是 Codex 内部契约，不是 OpenAI 公共 API 定义的字段。每次升级 Codex 时，应使用集中维护的
...
预设资源的 `shared_overrides` 以官方 Codex `0.145.0-alpha.20`
`gpt-5.6-terra` catalog 中 24 个客户端实际消费、且与模型身份无关的字段为起点。
Terra 在这里是 Codex 运行表面的参考，目标是通过 Rosetta 的 Chat 桥接适配当前最新的
第三方模型。共享值跟随 Terra 运行表面，只有 Rosetta 固定的 Responses Lite/Chat
转换确实需要时，才采用有意的协议差异值。
模型身份、上下文、模态、推理档位及其默认值、priority、`comp_hash` 和带身份的
指令内容仍由单个模型声明，或通过专用的身份替换路径生成。`shared_overrides` 中的
每个键也都允许写在单个 `models[]` 条目中，并由模型值覆盖共享默认值。
`template_slug` 只为 Rosetta 尚未识别的未来 catalog 字段提供向前兼容兜底；
客户端已知但未消费的字段以及新 catalog 已删除的字段都不会从模板中悄悄继承。
本段只记录模型 catalog 的局部快照，不代表已经完成 Codex `0.145` 的完整兼容适配。
...
下文把它们标记为 **0.144.4 未消费**。Rosetta 会从生成的第三方预设中省略它们，
...
| `available_in_plans` | 字符串数组，`["plus","team"]` | **0.144.4 未消费：**不在 `ModelInfo` 中，也不写入 Rosetta 第三方预设。 | 不要据此做 Rosetta 鉴权或路由；访问控制应在网关/provider 层实现。 |
| `minimal_client_version` | 打包 JSON 中的字符串，`"0.144.0"` | **0.144.4 未消费：**不在 `ModelInfo` 中，也不写入 Rosetta 第三方预设。 | 不要依靠它拒绝旧客户端；需要时使用明确的网关兼容策略。 |
...
| `reasoning_summary_format` | 字符串，`"experimental"` | **0.144.4 未消费：**不在 `ModelInfo` 中，也不写入 Rosetta 第三方预设。 | 不要按此字段分支 Rosetta 转换；应检查实际请求和流事件。 |
...
| `prefer_websockets` | 布尔值，`true` | **0.144.4 未消费：**不在 `ModelInfo` 中，也不写入 Rosetta 第三方预设；WebSocket 选择由其他位置控制。 | 不要根据这个字段宣称 WebSocket 支持或切换 Rosetta transport；应验证真实客户端请求路径。 |
