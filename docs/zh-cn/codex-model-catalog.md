...
| `supports_search_tool` | 布尔值，`false` | 控制原生 `tool_search` 等 Codex namespace/tool discovery；它与 hosted `web_search`、独立 `web.run` 不同。 | 只有客户端可见的发现流程经过测试时才启用。Responses→Chat Code Mode 路径会在实时 `exec` 描述声明 deferred tools 时，另行投影基于当前请求 `ALL_TOOLS` 的搜索与精确读取。成对的 Node REPL 精确读取会授权结构化 dispatcher 调用，再转回 `exec`；Rosetta 不保留 Gateway namespace/discovery cache。hosted search 在 Tool Profile 中配置，Modified `web.run` 使用全局联网搜索设置。 |
