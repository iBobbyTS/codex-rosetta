...
当 `web.run` 为“修改”时，即使 Tool Mapping only 路由仍把 `web__run` 放在 custom
...
**OpenAI Responses (Tool Mapping only)** 支持 Tool Profile，同时让 Responses 请求和响应的其余部分继续走直接路径。Responses Rosetta、Chat、Anthropic 和 Google 模型组也支持 Profile 选择与处理。打包的 Profile 包括 **Chat Default** 和 **OpenAI Responses Tool Mapping Only**；需要独立策略时，请先复制再修改。
