...
内置 **Chat Default** Profile 会将以下 Function 设为 Modified，并追加对应工具使用指导：
...
新版 Codex Code Mode 会把部分运行时工具放在 custom `exec` 的 description 中，而不再把每项工具都作为顶层 Function 暴露。对于 Responses 到 Chat 的路由，Chat Default 的投影规则会在声明实际存在且 Profile 状态为 Modified 时，把以下嵌套工具投影为普通 Chat Function：
