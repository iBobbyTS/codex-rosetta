...
| Code-to-document reverse map | Rebuilt from current Rosetta code and deterministic tests in [`rosetta-source-map.md`](rosetta-source-map.md); the stable ledger now contains 24 points, including the post-baseline late-developer cache-compatibility boundary |
...
| `0.142.0` … `0.145.0` source coverage | The prior range map and alpha.23 review were extended to the exact formal release; CP-01…CP-23 remain classified in the formal report, while CP-24 is a later daily-development addition with isolated live ESC/Steer cache evidence recorded in the appendix |
