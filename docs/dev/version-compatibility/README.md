...
| CP-15 daily-development sync | The existing Web search bridge point now owns ordered provider-chain replay, process-local cooldown identity, the fixed Responses search-model allowlist, single-Provider/single-Key routing, and the bounded Admin usage/test surfaces. This documentation update does not advance the Codex or Rosetta version and does not replace live search evidence |
