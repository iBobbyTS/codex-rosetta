...
- Whether one Codex SearchRequest remains safe for complete atomic replay across the ordered Provider chain; recheck the 32-row limit, one attempt per candidate, typed failover-only advancement, one-hour process-local cooldown identity/restart behavior, Tavily 432/433 quota classification, configured Responses single-credential binding, and bounded Admin usage/test DTOs;
...
- web search multi-round event reconstruction, zero-to-32 ordered Provider rows, legacy single-row normalization, duplicate/credential-overlap rejection, complete-request atomic replay, one attempt per stable candidate identity, typed candidate-health/request-local failover, one-hour process-local cooldown/reorder/restart/identity-change behavior, Tavily 432/433 quota classification, and bounded concurrency/state/request budgets. Deterministically verify that a configured Responses first row remains in the Modified coordinator and can fail over, Passthrough ignores all list rows and Search Models in favor of the current native route, and Disabled calls neither candidates nor native transport. Also cover downgrade paths for disabled/missing search executors, bounded auxiliary HTTP responses, Tavily and self-hosted result normalization into the unchanged Codex string `output`, configured-Responses Provider plus Search Model persistence/final override with exactly one credential, code-derived Admin row family/mode/capability plus full/local/mixed chain DTOs with no credentials or Tool Catalog dependency, allowlist parity with `responses_lite_model_fields`, five-minute coalesced plan-usage DTOs, controlled Admin Search Test errors, and standalone Search `turnXsearchY` allocation/open scope, retry stability, concurrency, TTL/capacity, and shutdown cleanup;
