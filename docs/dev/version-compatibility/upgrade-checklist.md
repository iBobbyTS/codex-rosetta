...
- Modified code-mode `web.run` derives from the live Codex declaration but advertises only Rosetta-supported commands and nested fields, removes unsupported command guidance, and keeps the same capability whitelist as runtime rejection; Passthrough retains the complete live declaration, while malformed Modified schemas fail closed;
