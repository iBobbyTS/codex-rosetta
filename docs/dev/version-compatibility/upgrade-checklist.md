...
- web search multi-round event reconstruction, downgrade paths for disabled/missing search executors, bounded identity-encoded auxiliary HTTP responses, Tavily and self-hosted Google result normalization into the unchanged Codex string `output`, configured-Responses Provider plus Search Model persistence and final upstream-model override, exact Search Model allowlist parity with the reviewed `responses_lite_model_fields` source contract, and standalone Search `turnXsearchY` allocation/open scoped by authenticated principal plus `SearchRequest.id`, including retry stability, concurrent allocation, cross-session failure, TTL/capacity cleanup, and app shutdown cleanup;
