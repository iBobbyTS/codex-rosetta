...
- Whether standalone Search and Images still use JSON `POST /v1/alpha/search`, `POST /v1/images/generations`, and `POST /v1/images/edits`, including their model field and required headers; compare the configured-Responses Search Model allowlist (`gpt-5.6-sol`, `gpt-5.6-terra`, `gpt-5.6-luna`) with extracted `responses_lite_model_fields` slugs and the current official latest-model result, and replace a reviewed entry only after Codex removes it or identifies its successor;
- Whether one Codex SearchRequest remains safe for one complete circular replay starting at the persisted current row; recheck the canonical 32-row limit and retired single-row rejection, one attempt per eligible candidate, success persistence, unsupported-sub-tool no-switch behavior, one-hour process-local cooldown/restart behavior, exact-zero persistent quota exclusion with hourly on-demand recheck, configured Responses single-credential binding, bounded Admin current/status/usage/test DTOs, and existing-window projection locking;
...
src/codex_rosetta/gateway/search_provider_candidates.py
src/codex_rosetta/gateway/search_provider_chain.py
src/codex_rosetta/gateway/search_provider_chain_state.py
src/codex_rosetta/gateway/search_provider_executor.py
src/codex_rosetta/gateway/search_usage.py
...
`2026-07-18` baseline; CP-24 added late instruction message cache compatibility;
CP-25 added window-scoped Chat tool surface stability; and CP-26 added
collaboration argument confidentiality and delivery mode, for 26 points now. A
missing, duplicated, or renamed row is a
...
- Compile Tool Catalog schema v7 at startup and require the reviewed 57 items, every exact source registration, and every dynamic family. Reject unknown registration forms, exposure values, source bindings, surface policies, adapters, dependencies, duplicate wire identities, and dangling families. The extractor must fail when a new Codex registration form cannot be classified. Runtime traces may contain only catalog/generation/epoch, hashes, counts, actions, and reasons—never schema, prompts, arguments, credentials, window IDs, or declaration bodies;
- Verify `x-codex-window-id` Chat surface locking with first-writer concurrency, restart recovery, sliding TTL, quota/integrity/wrong-key failures, strict live declaration matching, request-local search/read/hash authorization, stale-executor rejection, opaque rollover, and identical streaming/non-streaming final transport tools. Confirm non-Chat and windowless routes remain stateless;
- Verify model-group rows prefer an exact upstream-model slug and fall back to the exposed model name. Resolve the complete current Codex record through deep-copy plus recursive `model_info` overlay, then provider runtime-preset copy, exact upstream-first/exposed-fallback model-preset match, and `runtime_capabilities` overlay. Cover recursive object merge, whole-array replacement, scalar/null replacement, minimal deep diff against the matched Provider/model preset, empty override omission, complete unknown-preset validation, provider-declared `temperature`/`top_p` request overrides, rejection by undeclared providers, model-bound Admin auto-fill, yellow modified state, named restore, hidden complete-record preservation through the visual editor, the separate Provider-limits dialog, and one identical `ResolvedModelProfile` for local catalog generation and Gateway enforcement;
...
- Modified code-mode `web.run` derives from the live Codex declaration but advertises only Rosetta-supported commands and nested fields, removes unsupported command guidance, and keeps the same capability whitelist across top-level Functions, Responses→Chat projection, ordinary Tool Mapping only `exec.description`, Lite `additional_tools`, and runtime rejection. Test new-window projection from the persisted current row: an unready self-hosted current row must omit `search_query` even when a later configured Responses or Tavily row is eligible; a configured Responses current row, a Tavily current row with a valid Key, or a self-hosted current row whose sidecar reports `browser_ready=true` exposes its supported `search_query` schema. Later fallback rows do not widen that new-window surface. Existing windows retain their locked projection across Provider-state changes, and an invocation that is stale for the current Provider fails as currently unavailable. `open`/`time`/`response_length` are always retained, while `click`/`find`/`screenshot` require `browser_ready=true`. Verify the shared five-second health cache, concurrent refresh coalescing and hot-reload invalidation. Passthrough retains the complete live declaration and Disabled remains blocked; missing declarations are not invented; malformed Modified nested sections are removed without altering sibling exec sections;
- Chat Default Code Mode projection keeps the Disabled parent `exec` through Responses-to-Chat conversion, parses the live Codex declarations for `exec_command`, `write_stdin`, `update_plan`, `apply_patch`, `view_image`, `web.run`, conditional `image_gen`, Goal, Clock, Memories, Skills, and conditional deferred/environment/context/plugin/MCP-resource/Agent-Job tools; covers the Codex renderer's literal, union, intersection, array, tuple, object-index-signature, and normalized-heading forms; rejects unknown TypeScript tokens; emits ordinary Chat Functions only for valid model-visible declarations; never invents a tool when Codex's Feature, runtime, auth, environment, modality, or session-source gates omit it; removes only exact successfully replaced heading-through-declaration-fence spans and retains raw `exec` for deferred guidance, unknown or duplicate sections, malformed known sections, direct-name conflicts, and unparsed source formats; removes the parent only after all recognized sections are consumed; uses the matching `text(...)`, `image(...)`, or `generatedImage(...)` result helper; keeps `apply_patch` model-hidden while retaining its internal projection for localized `Edit`/`Write`, and consumes its description only when all catalog-declared replacements were emitted; rebuilds deterministic custom `exec` calls in streaming and non-streaming paths; preserves same-named direct Functions; keeps `wait`, `request_user_input`, `new_context`, and `collaboration` direct; restores collaboration namespaces; and replays projected call/result history from principal-scoped encrypted single-object translations;
- native/model-facing tool history objects with call/result-independent exact
  replay, current top-level ID injection, nested-ID preservation, principal-only
  ownership, domain-separated HMAC lookup, exact encrypted SQLite payloads,
  cross-window/model/Provider/fork reuse, and cross-principal isolation;
- authenticated same-process/restart replay, missing/wrong key and tamper
  fail-closed behavior, plaintext discard, atomic encrypted-v1 dedupe/conflict/
  expiry migration, row/principal/global row+byte budgets, accounting validation,
  non-renewing absolute 24-hour TTL, expiry retranslation and rewrite,
  transaction rollback, abnormal replay bounds, ordinary/JSON/error/cancelled
  results, parallel identical calls with reversed results, and subsequent-round
  replay;
...
- web search multi-round event reconstruction, zero-to-32 canonical Provider rows with retired single-row rejection, duplicate/credential-overlap rejection, persisted sticky-current selection, complete-request circular replay, one attempt per eligible stable candidate identity, successful-current persistence, unsupported-sub-tool no-switch behavior, one-hour process-local cooldown/reorder/restart/identity-change behavior, exact-zero persistent quota exclusion with hourly on-demand recovery checks, and bounded concurrency/state/request budgets. Deterministically verify that configured Responses rows remain in the Modified coordinator and can fail over after a Provider search failure, Passthrough ignores all list rows and Search Models in favor of the current native route, and Disabled calls neither candidates nor native transport. Also cover downgrade paths for disabled/missing search executors, bounded auxiliary HTTP responses, Tavily and self-hosted result normalization into the unchanged Codex string `output`, configured-Responses Provider plus Search Model persistence/final override with exactly one credential, code-derived Admin row family/mode/capability plus full/local/mixed chain DTOs with no credentials or Tool Catalog dependency, bounded Admin row ID/status/current DTO with manual cooling selection and exhausted rejection, allowlist parity with `responses_lite_model_fields`, five-minute coalesced plan-usage DTOs, controlled Admin Search Test errors, existing-window projection locking/new-window current capability projection, and standalone Search `turnXsearchY` allocation/open scope, retry stability, concurrency, TTL/capacity, and shutdown cleanup;
- Cross-format round trip of reasoning effort/summary/content/encrypted state, including exact OpenAI Responses and Chat `max` preservation, immediate Codex `light` to backend `low` normalization with no downstream `light`, nearest-supported-level warning, Provider-Profile-only wire construction, OpenCode Go Chat `reasoning_effort`/`reasoning_content`/cached-token normalization, and rejection of the removed model-specific reasoning mapper;
...
- With Tavily, Configured Responses, DeepSeek official Responses (`https://api.deepseek.com`, fixed `deepseek-v4-flash`, q-only/single-query), Self-hosted (Google), Self-hosted (Bing RSS), and Self-hosted (Bing Browser) tested separately, the model can initiate a search, read normalized results, and proceed to generate the final answer without a Codex response-shape difference. For DeepSeek, verify the UI consumes only eligible configured Provider names and exposes no Pro, domain, multi-query, history, usage, or quota state. Separately use a multi-row chain to prove current-first circular complete-request replay, one attempt per eligible candidate, successful-current persistence, no switch for an unsupported sub-tool, and existing-window projection stability; record Provider/model/route without raw errors or credentials.
