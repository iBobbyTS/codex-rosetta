...
- With Tavily, Configured Responses, Self-hosted (Google), Self-hosted (Bing RSS), and Self-hosted (Bing Browser) tested separately, the model can initiate a search, read normalized results, and proceed to generate the final answer without a Codex response-shape difference. Separately use a multi-row chain to prove ordered complete-request replay and one attempt per candidate after an eligible failure; record Provider/model/route without raw errors or credentials.
