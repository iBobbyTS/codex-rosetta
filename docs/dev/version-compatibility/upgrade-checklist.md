...
- request-local `ALL_TOOLS` discovery only with live deferred guidance; fixed `tool_search`, `tool_read`, and `invoke_deferred_tool` definitions beside raw `exec`; byte-identical top-level Chat `tools` across search/read/call; exact schemas and validation; natural-language/regex search JavaScript; versioned `{name, summary}` output with 240-character summaries and a 24,000-character whole-match budget; exact-name read JavaScript with a complete-declaration 24,000-character fail-closed budget; custom `exec` round trips; direct-name conflicts; and no Gateway discovery state. Verify search alone cannot authorize the dispatcher; paired read history authorizes only the exact three Node REPL names after same-name declaration parsing; latest same-name reads replace older outcomes while unrelated reads do not revoke authorization; `js` does not imply helper authorization; structured dispatcher arguments produce JSON-safe custom `exec` with text/image/`isError` forwarding in streaming and non-streaming paths; malformed, oversized, unpaired, fake-protocol, mismatched-name, unauthorized, non-object, legacy-v1, and direct-Node-call inputs fail closed; direct Node/read/dispatcher Functions take precedence. Verify search summaries and complete read results retain history order, unsupported declarations retain raw-`exec` instructions, and no discovered Node Function is added to top-level `tools`. Keep native `tool_search_call/output` converter fixtures separately as protocol compatibility coverage;
