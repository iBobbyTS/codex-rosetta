...
- Verify model-group rows prefer an exact upstream-model slug and fall back to the exposed model name. Resolve the complete current Codex record through deep-copy plus recursive `model_info` overlay, then provider runtime-preset copy plus `runtime_capabilities` overlay. Cover recursive object merge, whole-array replacement, scalar/null replacement, minimal deep diff, empty override omission, complete unknown-preset validation, the two allowed runtime fields, runtime ownership of corresponding catalog fields, yellow modified state, named restore, and one identical `ResolvedModelProfile` for local catalog generation and Gateway image/reasoning enforcement;
