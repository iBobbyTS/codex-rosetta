...
- Static tool-catalog contract: unique IDs, valid visible or explicitly UI-hidden placement/policy references, required fixed tools, direct Responses Namespace parents versus flat Code Mode `namespace__function` entries, normal/Code-Mode placement and conditional-exposure localization keys, Function/Hosted input IDs/types/defaults/localization keys, hidden and Chat-Default-disabled `test_sync_tool`, excluded runtime-dynamic plugin/MCP/app/connector tools and obsolete hosted `image_generation`, current `image_gen__imagegen` coverage, Chat Default Profile defaults, supported states, and exact CLI/source metadata binding.
