...
| `CP-15 — Web search bridge` | Changed | Custom Providers may opt into standalone `/alpha/search`; S03 enables `supports_standalone_web_search = true` only in the managed Rosetta local Provider TOML block, preserving arbitrary user Provider configurations. |
