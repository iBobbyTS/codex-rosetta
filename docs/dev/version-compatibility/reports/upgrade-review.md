...
| `CP-03 — Codex Search and Images endpoints` | Changed | `web.run` command/field/description projection is catalog-owned; obsolete `image_generation` suppression removed; `image_gen.imagegen` contract unchanged. Auxiliary credential streaming now uses bounded decoded/wire rolling windows for ordinary fields and full retention only for unfinished embedded JSON; no fragment-count capacity remains | Post-migration search passed for DeepSeek 3/3 and Terra 2/3 with Sol fallback for task 01. Qwen selected the exact image tool, but the configured Images route returned `404 model_not_found` for `gpt-image-2`. Current Search/Images production routes are non-streaming, so the new internal streaming behavior has deterministic transport coverage only and no new real call was authorized |
...
| `CP-17 — Stream lifecycle` | Changed | Model stream lifecycle no longer owns credential fragment/identity state. Deferred trace capacity remains diagnostic-only: a dropped trace batch cannot terminate the stream. The separate auxiliary streaming guard releases complete safe SSE events immediately, has no total fragment bound, and retains only unfinished structured arguments under byte/identity caps | Deterministic model raw SSE and auxiliary ordinary/structured streams with more than 4096 deltas complete; split credential collisions and state cleanup remain fail closed. The focused auxiliary/redaction cone reported `172 passed`; the full suite reported `3763 passed, 4 skipped`; lint and compatibility checks passed. No new live run was authorized |
