...
| `CP-24` | Hard-interrupt notice cache compatibility |
...
| Hard-interrupt notice cache compatibility | On an enabled Responses→Chat route, an authenticated Codex turn may convert only the exact canonical `<turn_aborted>` message from system/developer semantics to a separate user-role `<codex_runtime_notice>` with explicit non-user attribution. The marker and its meaning remain model-visible, the actual continuation user message remains separate, and Steer or arbitrary system content is unchanged. The abandoned stream follows ordinary cancellation: Rosetta does not drain it, retain hidden output, synthesize tool results, or replay content. Direct Responses and non-Chat protocols are unchanged. Any retired plaintext handoff table is deleted on startup | `gateway/interrupt_notice.py`, `gateway/app.py`, `gateway/provider_profiles.py`, `observability/persistence.py`, Admin Provider UI, `test_interrupt_notice.py`, `tests/live_agent/interrupt_continuation/`, `test_persistence_sqlite.py` | Codex changes `client_metadata["x-codex-turn-metadata"]`, request-kind identity, canonical `<turn_aborted>` wording or placement, or ESC/Steer scheduling; a Chat Provider changes role support or prompt-cache segmentation |
...
| Hard-interrupt notice cache compatibility | Validate DeepSeek/default/explicit Provider configuration, exact canonical metadata and marker parsing, copy-on-write user-role wrapping, multiple markers in order, Steer/no-marker/noncanonical skips, non-Chat/direct-Responses exclusion, old-table deletion, and unchanged ordinary stream cancellation | Through a temporary non-8765 Gateway, run `tests/live_agent/interrupt_continuation/run_live.py --mode interrupt` and `--mode steer` against DeepSeek Chat with one fresh random `user_id` per cell. Record per-request input/output/cached tokens when usage is present, target role/marker counts, Provider request count, and terminal events. Confirm ESC cancels the predecessor, the next request contains exactly one attributed user notice and no canonical middle system notice, its completed usage reports a non-zero cache hit, Steer contains neither notice form, and Rosetta sends no extra upstream request |
