...
| Responses→Chat bridge | Convert Codex Responses requests to Chat via IR, expand Function Namespace children as the regex-safe canonical `namespace-function`, and rebuild Responses output. Namespace non-Function children reuse the same loss-minimizing degradation as top-level non-Function tools, so a custom child remains a callable raw child container without changing ordinary Function prefix/restoration semantics. Chat source normalization treats the first finish for each choice as authoritative, so repeated upstream finish chunks rebuild one Responses item-done/completed sequence while retaining late usage. Synthetic `response.reasoning_summary_text.delta` events carry `summary_index: 0`, matching the single synthetic reasoning summary consumed by Codex. Response restoration also accepts `namespace_function`, `namespace.function`, or a bare child name only when exactly one Namespace owns it and no top-level Function has that name; ambiguous names remain flat. Codex `agent_message` input becomes Chat-visible user content, including its inter-agent `content[].encrypted_content`, while ordinary message/reasoning encrypted content remains opaque | `converters/openai_chat/converter.py`, `converters/base/context.py`, `converters/openai_responses/**`, `gateway/proxy.py` | High; new item/event/identity fields are not automatically transparent, Namespace child kinds or Function naming can become ambiguous, repeated finish ordering can duplicate a terminal tool item, and inter-agent payload carriers can change independently |
...
| Context compaction resilience | Remove orphan `tool_choice/tool_config` that has no tools but remains after compact; keep tool history replayable. Before Remote V2 preparation, emit an always-on, prompt-free request shape with fixed input-type buckets and distinct absent/true/false/invalid `parallel_tool_calls` state. Compaction-specific preparation, internal-summary, persistence, and native upstream failures emit self-contained safe warnings without request content, tool schemas, opaque tokens, arbitrary reason values, or upstream/exception text; the internal summary has its own shape and suppresses the generic raw upstream-error record | `gateway/codex_compaction.py`, `gateway/logging.py`, `gateway/proxy.py`, `converters/base/helpers/tool_orphan_fix.py`, `test_codex_compaction.py`, `test_body_logging.py`, `test_strip_orphaned_tool_config.py` | Codex compact output, trigger/replay item types, reason values, internal-summary request shape, upstream failure lifecycle, window generation or historical clipping changes |
...
| Stream lifecycle | created, item/delta/done, completed/failed/incomplete sequence; per-choice first-finish idempotence with independent multi-choice completion; repeated Chat finish with late usage produces one finish/item-done/completed, one usage, and one stream end; empty-choice usage remains terminal; exact error-origin labeling; incomplete-to-failed mapping; protocol-valid late terminal errors; huge declared HTTP chunks; no-newline/no-delimiter SSE; converted/raw/web-search overflow and early-close classification; complete anonymous-spool release for safe deferred response diagnostics beyond 1 MiB/4096 records; discard and cleanup on failure, cancellation, and `aclose()`; protocol-field-only response diagnostics with ordinary-error retention | Real streaming turn, including the 0.149 repeated-finish-with-late-usage sequence, upstream interruption, and client disconnect without duplication/truncation/stuck; Codex must receive exactly one terminal tool item sequence with final usage, and receive the labeled failure instead of synthesizing unlabeled EOF; more than 4096 ordinary deltas must still complete, and an enabled trace must retain the complete safely finished response diagnostics and terminal usage regardless of stream length |
...
For converted Chat streams, the first `finish_reason` for each choice is
authoritative. A repeated finish for that choice emits no second IR finish or
content-block end, but a repeated chunk carrying final usage still emits the
single usage and stream end needed to complete the deferred Responses payload.
The composed Chat-to-IR-to-Responses regression verifies one arguments-done,
one output-item-done, and one completed event for the call.
