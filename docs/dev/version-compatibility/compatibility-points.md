...
| Codex model catalog | Diff the complete bundled JSON key set and per-model values; verify the packaged upstream asset byte-for-byte; validate exact upstream-first/exposed-fallback preset matching, complete `model_info` preservation, recursive merge/diff, whole-array replacement, explicit null, unknown-preset validation, provider runtime-preset inheritance, the two allowed runtime fields, yellow modified state, named restore, and identical Codex/Gateway `ResolvedModelProfile` use. Test local-mode defaults, custom Codex Home, CLI/WebUI lifecycle, stable alias generation, TOML preservation, managed cleanup, mutation synchronization, key non-rotation, Provider replacement, rollback, and package resources | Start Codex against generated automatic and manually edited catalogs; confirm the selected alias, complete display metadata, Provider identity and bearer key. Restart after a WebUI model/runtime mutation and confirm the canonical minimal config diff and matching Gateway modality/reasoning enforcement. For every preset selector, run the actual third-party upstream and inspect Gateway Logs for command/code mode, Responses Lite, reasoning, context/compact, image input, search, collaboration v2, and OpenAI-identity compact behavior |
...
`model_info`. The only runtime override fields are `input_modalities` and
`supported_reasoning_levels`; they own the effective value while present.
Admin marks any canonical diff in yellow and restore removes it. The resulting
single `ResolvedModelProfile` supplies both local Codex catalog generation and
Gateway capability enforcement. Upgrade checks must compare the complete
catalog record and both runtime fields, not a fixed eight-field subset.
