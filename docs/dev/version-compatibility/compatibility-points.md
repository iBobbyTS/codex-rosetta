...
| Web search bridge | Codex `web_search` event reconstruction remains supported, while Modified `web.run` resolves an ordered `server.web_search.providers` list of at most 32 Tavily, configured Responses, or self-hosted rows. Each SearchRequest replays its complete atomic query set against eligible candidates in configured order, once per candidate, and advances only for typed candidate-health or request-local failover outcomes; 400/422 request rejection is terminal, does not advance, and starts no cooldown. Candidate-health failures publish a one-hour process-local cooldown keyed by stable row/configuration/credential identity; restart clears state, reorder preserves it, identity changes are fresh, and Tavily 432/433 are quota exhaustion. Only Modified consults this list: a configured Responses first row still runs through the coordinator and can fail over; Passthrough ignores every list row and Search Model and uses the current native Responses route; Disabled blocks before either chain or native transport. Each configured Responses row binds one enabled `responses` Provider, exactly one Provider credential, and one reviewed Search Model (`gpt-5.6-sol`, `gpt-5.6-terra`, or `gpt-5.6-luna`); under Modified it uses the app-owned bounded Provider transport to forward to relative `alpha/search`, retains proxy, redirect, and public request headers, and overwrites the final upstream model without conversation-route leakage. Provider runtime/Admin are single-Key; legacy comma-separated keys select the first non-empty value only at the read boundary and converge on save. Self-hosted rows have no quota or credential fallback and never substitute another engine within a row; typed candidate-health failure may still advance to a later explicitly configured self-hosted row. Tavily Admin usage exposes only `account.plan_usage`/`account.plan_limit`, caches and coalesces for five minutes, and reports only a month-start reset date because exact reset time/time zone are unavailable. Admin Search Test delegates to the public `alpha/search` handler, renders bounded result cards, and maps failures to controlled categories without Provider raw errors. The existing five-second sidecar-health cache and relative Base URL semantics remain unchanged | `gateway/config.py`, `gateway/providers.py`, `gateway/search_provider_candidates.py`, `gateway/search_provider_chain.py`, `gateway/search_provider_chain_state.py`, `gateway/search_provider_executor.py`, `gateway/search_usage.py`, `gateway/web_search.py`, `gateway/codex_search.py`, `gateway/codex_auxiliary.py`, `gateway/admin/routes/config.py`, `gateway/admin/routes/network_search.py`, `gateway/web_run_sidecar.py`, `gateway/transport/http/transport.py`, `test_config.py`, `test_providers.py`, `test_search_provider_candidates.py`, `test_search_provider_chain.py`, `test_search_provider_chain_state.py`, `test_search_provider_executor.py`, `test_search_usage.py`, `test_codex_auxiliary.py`, `test_admin_config_routes.py`, `test_admin_network_search_routes.py`, `test_codex_source_contract.py`, WebUI network-search/API/page tests | native web-search item/event; Provider-list schema/order/limit; candidate identity, replay, failure taxonomy, cooldown or budget semantics; single-Key migration; selected Responses Provider/model validation or allowlist; Admin usage/test DTO; auxiliary response behavior; Base URL, health-cache, or sidecar semantics change |
...
| Web search bridge | Validate zero through 32 canonical ordered rows; legacy single-row migration; strict row/provider/model/key validation; candidate identity and duplicate/credential-overlap rejection; complete-request atomic replay; one attempt per candidate per SearchRequest; typed candidate-health and request-local failover only; one-hour process-local cooldown, reorder stability, identity-change freshness, restart reset, Tavily 432/433 quota classification, and bounded state/concurrency/budget behavior. Verify the unchanged Passthrough/Disabled semantics, exact three-option Responses model allowlist, final upstream-model override, single Provider/single Key convergence, configured Responses and self-hosted boundaries, five-minute Tavily usage cache/coalescing exposing only plan usage/limit plus month-start reset date, and Admin Search Test delegation/result-card/controlled-error behavior | Exercise a multiple-row chain through real Codex and verify ordered full-request replay, one attempt per candidate, selected Responses model and Provider credential, visible recovery after an eligible failure, and no raw Provider error disclosure. Test all three reviewed Responses models separately. Record that live Provider/sidecar/network evidence is required; deterministic tests alone do not approve compatibility |
