...
| Protocol-fixed tool policy | Responses→Responses remains byte-preserving passthrough; Responses→Chat always localizes code tools, restores `apply_patch`, removes `image_generation`, optimizes tool descriptions and detects phases | `gateway/proxy.py`, `gateway/tool_adaptation.py` | Codex tool schema, catalog behavior, call/output format or phase semantics change |
