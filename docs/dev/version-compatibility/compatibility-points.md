...
| Deferred tool discovery | Temporary `namespace` tools by authenticated Codex window, inject/process `tool_search`, restore `tool_search_call/output`, and atomically enforce 1,024 tools/16 MiB per scope, 256 unique scopes per principal, 1,000 scopes per retained map, and 64 MiB per app without cross-principal eviction. Tool Profiles continue to own eager static catalog namespaces, while runtime plugin/MCP tools returned in `tool_search_output` still enter the same window store and are injected into Responses-to-Chat requests | `gateway/proxy.py::WindowToolSearchStore`, `_prepare_window_tool_search_request`, Responses converter | namespace/tool_search schema, execution, plugin activation/exposure, Tool Profile ownership, compact behavior, or retained payload size/window fan-out changes |
...
| Deferred tool and capability discovery | namespace defer, search matching, multiple searches, call/output, profile-route runtime discovery with static-profile eager ownership unchanged, ordered skill/plugin contextual metadata, explicit skill/plugin injection, implicit selected-skill read, plugin provenance, two-way window isolation, UTF-8 byte/count budgets, per-principal unique-scope quota, same-principal global-oldest eviction, no cross-principal eviction, atomic overflow, concurrency, TTL/eviction/clear accounting | Run `tests/live_agent/deferred_tool_search/01` through `07`: three explicit controls plus standalone-skill, standalone-MCP, plugin-skill, and plugin-MCP natural-language discovery. Require ordered skill/plugin context in rollout and converted requests; for MCP, require the `exec` discovery contract in source/target requests plus an ordered three-candidate runtime `ALL_TOOLS` catalog and selected-call provenance. Also require no implicit-prompt identifier leakage, only the selected archive body/tool, a consumed result, and Terra completion before any DeepSeek cell |
...
For a Responses-to-Chat route with a Tool Profile, the Profile remains the
owner of static catalog namespaces and Rosetta does not move those namespaces
into the deferred store. Runtime plugin/MCP tools are different: Codex returns
them inside `tool_search_output`, outside the static catalog. The window store
must therefore retain and inject those discovered tools even when the route has
a non-empty Profile. Gateway tests cover both halves of this ownership boundary.
The controlled CLI run on 2026-07-16 installed and listed the local fixture MCP,
but after three bounded attempts did not expose `tool_search` or the plugin MCP
tool to the model; this real-plugin gate remains open and must not be counted as
verified until the isolated rollout shows search, call, and consumed result.
