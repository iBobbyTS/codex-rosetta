...
| Codex Search and Images endpoints | Expose JSON `POST /v1/alpha/search`, `/v1/images/generations`, and `/v1/images/edits`. Images and unbridged Search retain direct Responses Pass-through routing. Standalone Search locally implements atomic `search_query` through Tavily, direct-public-URL static HTML/plain-text `open` through a bounded Python fetcher, and fixed-offset `time`. Open revalidates public addressing on every redirect and returns normalized line-addressable text; stored references, browser rendering, compressed/non-text pages and the remaining unsupported commands/settings fail as structured HTTP 501 before partial execution. Gateway Logs record local request/result/error stages | `gateway/codex_auxiliary.py`, `gateway/codex_search.py`, `gateway/codex_page.py`, `gateway/app.py`, `routing.py`, `test_codex_search.py`, `test_codex_page.py`, `test_codex_auxiliary.py`, `test_downstream_routes.py` | Codex changes endpoint paths, SearchRequest commands/settings, required headers, SearchResponse shape, open reference/line semantics, or no longer includes routing model/session identity |
...
| Codex Search and Images endpoints | Verify all three POST routes, model validation/aliasing, native pass-through path/body/status/header/error behavior, local Tavily search, bounded static direct-URL open and Python time behavior, public-address/redirect/content-type/size/line handling, domain/context/length mapping, atomic mixed-command rejection, stable `not_implemented` errors, and Gateway Logs stages | Invoke standalone `web.run` through both a pass-through relay and a converted third-party model with Tavily configured; verify successful search, direct-URL open and time results, plus fatal 501 for stored-reference/browser navigation. Invoke `image_gen.imagegen` through Responses Pass-through and confirm generation/edit forwarding |
