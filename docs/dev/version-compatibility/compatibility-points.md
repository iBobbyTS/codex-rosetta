...
| Model-group tool profiles | Validate all bundled/user Profile contracts, Admin CRUD and reference guards, bundled input-only override validation, model-group resolution for every protocol, per-tool filtering, multi-value Function/Hosted text/password/select input persistence, select-option validation, input/description state visibility, namespace pass-through/expansion, injected-tool selection, absence of hosted `image_generation`, and `web.run`/`image_gen.imagegen` endpoint selection | Use real Codex sessions on Responses pass through, Responses web.run mapping, Chat Default, and a restrictive user Profile; verify bundled card fields survive explicit save/reset while their delivery states reject edits, direct tools remain unchanged in the first preset, only `web.run` selects local Search in the second, card inputs survive copy/save/reset and retain values while hidden, descriptions default to all states and honor overrides, search cards use their own Tavily Tokens, select labels display while values persist, disabled tools disappear, Modified `image_gen.imagegen` selects its OpenAI Images endpoint without leaking its token, and supported adaptations behave as documented |
