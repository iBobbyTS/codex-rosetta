...
new source-tool item. Catalog schema v6 contains 57 owned entries: the 53
reviewed source concepts plus `hosted.tool_search`, `send_line`, `tool_read`,
and `invoke_deferred_tool` as explicit Profile/runtime contracts.
...
| --- | --- | --- | --- |
| Agent-facing API | Expose `/v1/responses` to Codex; Chat/Anthropic/Google as upstream target format; accept full-history image sessions under a configurable 64/128/256/512/1024 MiB or unlimited inbound body limit, defaulting to 128 MiB. Authenticated `/v1` Zstd bodies are decoded before JSON parsing, with the same live WebUI limit independently enforced before and after decompression. Responses→Responses direct transport copies unknown end-to-end client headers by default, then applies a case-insensitive denylist for client credentials, hop-by-hop/framing fields (including names declared by `Connection`), and network-origin identity. Rebuilt JSON also removes `Content-Encoding` and `x-oai-attestation`, forces JSON plus identity encoding, and sends the validated/generated `x-request-id`; unchanged raw wire preserves the original encoding, attestation, and only a client-supplied request ID. Provider auth is overlaid last and wins every case variant. Responses→Chat/Anthropic/Google retain the explicit minimal header set. Every Codex version review must diff source plus a real capture against this contract | `gateway/app.py`, `gateway/headers.py`, `gateway/inbound_content_encoding.py`, `gateway/config.py`, Admin config/UI, `gateway/proxy.py` | Codex changes endpoint, transport, request shape, request compression, attestation headers, end-to-end capability headers, or retained image/history size |
| Responses transparent handling | Admin exposes one `responses` wire protocol; unsupported former protocol values are rejected during configuration loading. Every Responses→Responses route is direct regardless of Provider: the gateway applies the selected Tool Profile, retains other request fields, and returns original credential-free response JSON/SSE bytes. The direct header set is computed once from the original inbound request, independently of Tool Profile, alias, and rehydration body processing, so ordinary allowed headers such as `x-codex-beta-features` survive a body change. Credentials, fixed and `Connection`-declared hop-by-hop fields, framing, and network-origin identity are removed case-insensitively; unknown headers pass only on this direct route. Provider auth is applied last with case-insensitive replacement. Any credential configured for the active Provider and reflected by that upstream fails closed before client, model, trace, log, metric, persistence, or semantic conversion use; credentials configured only for another Provider are intentionally outside this return gate, while global diagnostics still redact the complete configured-token inventory. No minimum credential length is assumed. Key rotation and return collision detection share the active Provider's canonical trimmed, non-empty credential sequence; raw CSV values remain diagnostic secrets too. Raw JSON and complete SSE `data` payloads combine exact-wire checks with parsed JSON string semantics, so equivalent Unicode, surrogate, and solidus escapes cannot bypass collision detection. For Responses text SSE that Codex concatenates, semantic-gate buffers use Codex's active output-item lifecycle plus only the summary/content index retained for that event; discarded delta wire IDs cannot split one consumer stream, and every stream terminal path clears the bounded state. Raw passthrough retains one bounded complete SSE event, releases only events proven credential-free across arbitrary HTTP chunks and event boundaries, and emits a source-compatible terminal error from a valid SSE boundary on collision. Model-switch compaction is the sole semantic request exception: it uses the previous model with Rosetta's prompt, stores a principal-scoped plaintext replacement for seven days, returns an `rskc_v1_` handle, and rehydrates it before the next Provider request. Native `context_limit` and `user_requested` remain unchanged. Authenticated Zstd ingress is decoded once for validation and routing; when an attested streaming request remains exactly equal to the parsed ingress object after all gateway processing, the transport forwards its original compressed bytes, original encoding, opaque attestation, and allowed direct headers. Any Profile, alias, rehydration, or cross-protocol mutation falls back to rebuilt JSON and, as a conservative trust-boundary policy, omits the original attestation and content encoding without otherwise recutting the direct header set. Public Codex source exposes the attestation token as an opaque just-in-time value and supplies only `thread_id` to its provider; it does not establish whether the token is bound to request bytes, a semantic body, headers, a nonce, or only client/device state. The upstream effect of retaining that token after body reconstruction is therefore **unknown**, not a proven stale-token rejection rule. If rebuilt and exact-wire requests diverge in status, routing, compaction mode, or retry behavior, investigate attestation retention/removal and body/header coupling before attributing the result to model or converter behavior. The response transport enforces 1 MiB per line and 8 MiB per event with no total successful-stream cap | `gateway/config.py`, `gateway/headers.py`, `gateway/inbound_content_encoding.py`, `gateway/tool_profiles.py`, `gateway/codex_compaction.py`, `routing.py`, Admin provider UI, `gateway/proxy.py`, `gateway/transport/provider_info.py`, `gateway/transport/credential_redaction.py`, `gateway/transport/credential_semantics.py`, `gateway/transport/http/transport.py`, `observability/redaction.py`, `test_responses_passthrough.py`, `test_provider_return_redaction.py`, `test_transport_credential_redaction.py`, `test_providers.py`, `test_admin_config_routes.py`, `test_tool_profiles.py`, `test_inbound_content_encoding.py`, `test_config.py`, `test_http_transport_limits.py` | A direct-header denylist omits a new credential or topology field, a conversion route accidentally enables fail-open forwarding, a Gateway hook mutates the request without forcing JSON fallback, a Provider-return path bypasses active-provider credential collision handling, an attestation with unknown scope is paired with rebuilt bytes without an explicit policy decision, the model-switch mapping cannot be rehydrated, Codex changes Responses output-item or text-delta identity semantics, changes request content encoding or attestation headers, emits a required single line or event above the safety envelope, or changes away from HTTP/SSE framing |
...
| Responses→Chat bridge | Convert Codex Responses request to Chat via IR, expand Namespace children as the regex-safe canonical `namespace-function`, and then rebuild Responses output. Synthetic `response.reasoning_summary_text.delta` events carry `summary_index: 0`, matching the single synthetic reasoning summary consumed by Codex; the final-source semantic gate rejects a missing or conflicting consumer identity as a response-contract error rather than misreporting a credential collision. Response restoration also accepts `namespace_function`, `namespace.function`, or a bare child name only when exactly one Namespace owns it and no top-level Function has that name; ambiguous names remain flat so Codex fails closed. Codex `agent_message` input becomes a Chat-visible user message, including its inter-agent payload carried in `content[].encrypted_content`, while ordinary message/reasoning encrypted content remains opaque | `converters/openai_responses/**`, `gateway/proxy.py`, `gateway/transport/credential_redaction.py`, `gateway/transport/credential_semantics.py` | High; new item/event/fields will not be automatically transparently transmitted, Codex may change the required identity fields for reasoning-summary deltas, Namespace or Function naming changes can create ambiguous restore candidates, and inter-agent payload carriers can change independently of ordinary messages |
...
| Code tool localization | Catalog schema v6 owns the complete model-visible definitions for `Read`/`Edit`/`Write`/`Glob`/`Grep` and `send_line`; runtime adapters only translate validated calls back to Codex-native tools. `Edit` and `Write` explicitly require preference over Shell/Python for file mutation. Chat Default projects selected nested `exec_command`, `write_stdin`, `update_plan`, `view_image`, `web.run`, Goal, Clock, Memories, Skills, conditionally `image_gen`, and conditionally assembled deferred/environment/context/plugin/MCP-resource/Agent-Job tools from Code Mode `exec`, while preserving same-named direct Functions. When effective native `write_stdin` and enabled `send_line` are both present, Rosetta exposes `send_line(session_id, line)` and reconstructs exactly one real newline; the `exec_command`/`write_stdin` hints are conditional catalog description fragments and disappear with the facade. `view_image` modality policy, unknown-modality fail-open behavior, eager-only delivery, and supported detail enum are catalog declarations. `web.run` base/search/browser command and field projections and description edits are likewise catalog capability data; runtime code supplies only current capability IDs. A Feature- or runtime-gated declaration that Codex omitted is never otherwise synthesized. The parent `exec` remains an internal container as declared by catalog delivery metadata. Model-facing `apply_patch` remains disabled while its internal projection supports localized edits. `Bash` is not exposed and survives only as the historical alias of `exec_command`; the obsolete hosted `image_generation` suppression has been removed. Parse failures, missing projections, duplicate sections, and direct-name conflicts fail closed | `gateway/tool_catalog_contract.py`, `gateway/tool_runtime_plan.py`, `gateway/code_mode_projection.py`, `gateway/web_run_capabilities.py`, `gateway/tool_adaptation.py`, `gateway/proxy.py`, `gateway/tool_profiles.py`, `gateway/admin/tool_catalog.json`, `test_tool_runtime_plan.py`, `test_code_mode_projection.py`, `test_tool_adaptation.py`, `test_tool_profiles.py`, `test_admin_tools_catalog.py` | Tool name, catalog schema/adapter contract, exec declaration syntax, helper output contract, direct-versus-nested availability, modality metadata, conditional assembly, capability IDs, or Profile state changes |
...
| Deferred tool discovery | Catalog schema v6 owns `tool_search`, `tool_read`, and `invoke_deferred_tool` definitions, delivery rules, dependencies, and adapter bindings. Modified `tool_search` is Chat-only and appears only when the live `exec.description` contains deferred/`ALL_TOOLS` guidance. Passthrough preserves native Responses wire; on Chat it projects only an actually present client-executed native declaration and reversibly restores `tool_search_call` plus paired `tool_search_output`. Disabled removes it. `tool_read` requires effective Modified `tool_search` plus deferred guidance, and `invoke_deferred_tool` additionally requires effective `tool_read`. Search/read/dispatch continue through deterministic custom `exec` JavaScript over the request-local `ALL_TOOLS` array, paired-read authorization, and fail-closed direct-name precedence. Missing execution metadata, malformed definitions, orphan call/output history, and mapping conflicts are rejected | `gateway/tool_catalog_contract.py`, `gateway/tool_runtime_plan.py`, `gateway/tool_search_bridge.py`, `gateway/code_mode_projection.py`, `gateway/tool_adaptation.py`, `converters/openai_responses/tool_ops.py`, `test_tool_runtime_plan.py`, `test_code_mode_projection.py`, `test_tool_adaptation.py`, `tests/converters/openai_responses/test_tool_ops.py` | Native tool-search schema/execution/history changes, deferred guidance changes, `ALL_TOOLS` format changes, custom `exec` semantics, MCP declarations, or direct-name conflicts |
...
| Static tool catalog | Package schema v6 with 57 catalog-owned entries, policy capabilities, complete injected definitions, runtime adapter IDs, availability predicates, delivery rules, description variants, capability projections, history aliases, state/API restrictions, placement metadata, and immutable bundled Profile source. The metadata is bound to Codex CLI `0.145.0` and source commit `25af12f7e61572b0bc18ddb1008be543b91519b0`. The 53 reviewed source concepts remain represented, while `hosted.tool_search`, `injection.rosetta.send_line`, `injection.rosetta.tool_read`, and `injection.rosetta.invoke_deferred_tool` make previously program-owned behavior explicit. `Bash` is a history alias only. Obsolete hosted `image_generation` and runtime-dynamic MCP/plugin/app/connector tools remain excluded. Startup compilation rejects unknown fields/adapters/dependencies/state combinations, duplicate identities, and cycles | `src/codex_rosetta/gateway/admin/tool_catalog.json`, `gateway/tool_catalog_contract.py`, `gateway/admin/tool_catalog.py`, `test_tool_runtime_plan.py`, `test_admin_tools_catalog.py` | Catalog schema, fixed tool sets, delivery/adapter identity, conditional exposure, state/API restrictions, aliases, placement, wire types, or model-controlled availability change |
...
Converted returns have two credential boundaries. The transport gate applies
the target provider's consumer identities before conversion. After target-to-IR
and IR-to-source conversion, a second gate applies the final source provider's
consumer identities before the document is body-logged/serialized or before a
stream event is traced/serialized. For Codex Responses this means document
`output_text` aggregation plus streaming active-output-item lifecycle and only
event-specific retained indices. Stream response diagnostics are held within a
1 MiB / 4096-record request-local budget and released only after safe completion
plus active-provider and global configured-token diagnostic aggregation;
collision, error, cancellation, early close, or overflow discards the batch.
Terminal exceptions pass the same active/global ordered-fragment checks before
stream finalization and outer instrumentation: unsafe detail is replaced by one
stable cause-free error before Trace, RequestLog, Metrics, or SQLite persistence,
while ordinary error detail remains available. Ordinary, raw passthrough, and
web-search generators all clear bounded gate and diagnostic state.
...
| Stream lifecycle | created, item/delta/done, completed/failed/incomplete sequence; huge declared HTTP chunks; no-newline/no-delimiter SSE; converted/raw/web-search overflow and early-close classification; target/final-source credential-gate cleanup; deferred response diagnostic release/drop/capacity cleanup on completion, failure, cancellation, and `aclose()`; active-provider and inactive/global sibling terminal-detail reconstruction across Trace, RequestLog, Metrics, and memory/SQLite with ordinary-error retention | Real streaming turn and client disconnect without duplication/truncation/stuck; terminal, cancellation, limits, and errors are presented correctly |
