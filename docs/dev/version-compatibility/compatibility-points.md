...
| `CP-24` | Late developer message cache compatibility |
...
| Late developer message cache compatibility | Validate DeepSeek/default/explicit Provider configuration, Codex metadata gating, leading-prefix preservation, positional copy-on-write conversion of all later developer messages, string and multipart `<system>` wrapping, malformed/late-system/user skips, non-Chat/direct-Responses exclusion, old-table deletion, and unchanged ordinary stream cancellation | Through a temporary non-8765 Gateway, run `tests/live_agent/interrupt_continuation/run_live.py --mode interrupt` and `--mode steer` against DeepSeek Chat with one fresh random `user_id` per cell. Record per-request input/output/cached tokens when usage is present, target role/marker counts, Provider request count, and terminal events. Confirm ESC cancels the predecessor, the next request contains exactly one user-role `<system>` envelope and no canonical middle system notice, its completed usage reports a non-zero cache hit, Steer contains neither notice form, and Rosetta sends no extra upstream request. Exercise fork/plugin/skill late-developer shapes deterministically because the production rule does not inspect their text |
