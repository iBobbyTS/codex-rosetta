...
| Web search bridge | Web Search Providers are independent code-owned contracts/adapters; the static Tool Catalog owns ordinary tool declarations only and must not duplicate provider sub-capabilities, request fields, validation, or invocation. Modified `web.run` resolves an ordered `server.web_search.providers` list. The Admin DTO derives non-sensitive per-row family/mode/capability data and the chain projection from the same code contract, never from persisted derived fields or Tool Catalog data. An all-configured GPT chain forwards every non-empty `commands` request body unchanged to relative `alpha/search`, where GPT owns upstream validation and the complete body/result response; Tavily/self-hosted use code-owned local query adapters and normalized references. The DeepSeek official Responses row consumes only eligible configured names from `web_search_contract.deepseek_providers`, uses the official `https://api.deepseek.com` origin, fixed `deepseek-v4-flash`, and one `search_query.q` per request; domains, recency, location, multi-query, Pro, history, usage, and quota state are out of scope. Candidate cooldown, budget, and typed error semantics remain as described above, including ordered chain fallback. | `gateway/search_provider_contract.py`, `gateway/search_provider_candidates.py`, `gateway/search_provider_executor.py`, `gateway/codex_search.py`, `gateway/codex_auxiliary.py`, `gateway/web_search.py`, `gateway/admin/routes/config.py`, `webui/src/admin/pages/NetworkSearchPage.svelte`, `test_search_provider_executor.py`, `test_codex_auxiliary.py`, `test_admin_config_routes.py` | Provider contract/adapter, Admin DTO shape, upstream alpha/search validation, candidate chain/error/cooldown/budget semantics, or local normalized-reference behavior changes |
