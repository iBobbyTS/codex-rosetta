...
| Model-group tool profiles | Validate all bundled/user Profile contracts, Admin CRUD and reference guards, model-group resolution for every protocol, per-tool filtering, multi-value Function text/password input persistence, namespace pass-through/expansion, injected-tool selection, absence of hosted `image_generation`, and `web.run` endpoint selection | Use real Codex sessions on Responses pass through, Responses web.run mapping, Built-in, and a restrictive user Profile; verify direct tools remain unchanged in the first preset, only `web.run` selects local Search in the second, Function inputs survive copy/save/reset without affecting runtime unless consumed, disabled tools disappear, `image_gen.imagegen` follows its namespace state, and supported adaptations behave as documented |
