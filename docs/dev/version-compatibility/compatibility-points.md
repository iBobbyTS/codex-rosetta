...
| `CP-24` | Soft-interrupt cache continuity |
...
| Responses→Chat bridge | Convert Codex Responses requests to Chat via IR, expand Namespace children as the regex-safe canonical `namespace-function`, and rebuild Responses output. Synthetic `response.reasoning_summary_text.delta` events carry `summary_index: 0`, matching the single synthetic reasoning summary consumed by Codex. Response restoration also accepts `namespace_function`, `namespace.function`, or a bare child name only when exactly one Namespace owns it and no top-level Function has that name; ambiguous names remain flat. Codex `agent_message` input becomes Chat-visible user content, including its inter-agent `content[].encrypted_content`, while ordinary message/reasoning encrypted content remains opaque | `converters/openai_responses/**`, `gateway/proxy.py` | High; new item/event/identity fields are not automatically transparent, Namespace or Function naming can become ambiguous, and inter-agent payload carriers can change independently |
...
| Soft-interrupt cache continuity | On eligible streaming Responses→Chat routes, a Chat Provider may drain the already-open upstream request after Codex ESC disconnects, capture only ordered `response.output_item.done` items from a normally completed detached stream, and persist one plaintext 24-hour handoff per authenticated principal plus exact `thread_id`. A later different `turn_id` with a real `<turn_aborted>` marker waits for that drain and replays only an exact Provider/model/cache-key/request-shape/source-prefix/Chat-prefix match. The marker remains a Codex-side validation boundary but is removed from the Provider-visible replay, so the continuation has the same cached-prefix shape as Steer. Steer, normal display, error, timeout, capacity overflow, fork ancestry, and direct Responses never replay. Missing function/custom results use the exact cancellation result and successful delivery consumes the row once | `gateway/soft_interrupt.py`, `gateway/app.py`, `gateway/proxy.py`, `gateway/provider_profiles.py`, `observability/persistence.py`, Admin Provider UI, `test_soft_interrupt.py`, `test_soft_interrupt_proxy.py`, `tests/live_agent/interrupt_continuation/`, `test_persistence_sqlite.py`, `test_web_search_bridge.py` | Codex changes `client_metadata["x-codex-turn-metadata"]`, request-kind/turn/thread/session/fork identity, `<turn_aborted>` history placement, ESC disconnect behavior, output-item completion ordering, prompt-cache ownership, or Steer scheduling; a Provider changes incomplete-stream cache guarantees |
...
streams pass ordinary text unchanged. Deferred stream diagnostics retain a
1 MiB / 4096-record request-local capacity only as a logging bound: overflow
drops the response trace batch and never changes or terminates the model stream.
...
| Soft-interrupt cache continuity | Validate DeepSeek/default/explicit Provider configuration, canonical metadata parsing, bounded independent drain, normal/error/incomplete/capacity outcomes, 24-hour TTL and byte quotas, partial item replay, exact source and converted target prefixes, synthetic function/custom cancellation results, one-shot deletion, steer-like marker removal, Steer/no-marker and identity mismatch skips, parent/fork/multiple-fork/subagent isolation, gateway shutdown, and no second local Web Search/upstream round after detach | Through a temporary non-8765 Gateway, run `tests/live_agent/interrupt_continuation/run_live.py --mode interrupt` and `--mode steer` against DeepSeek Chat. Record per-request input/output/cached tokens when usage is present, prompt-cache key/prefix continuity, displayed output, hidden handoff count, and terminal event; do not infer a hit rate when an interrupted stream has no usage. Confirm ESC drains once, removes only the control marker before the next same-thread Provider request, Steer remains visible and starts no handoff, a fork cannot consume its parent, and a fork's own later ESC remains independently replayable |
| Stream lifecycle | created, item/delta/done, completed/failed/incomplete sequence; exact error-origin labeling; incomplete-to-failed mapping; protocol-valid late terminal errors; huge declared HTTP chunks; no-newline/no-delimiter SSE; converted/raw/web-search overflow and early-close classification; deferred response diagnostic release/drop/capacity cleanup on completion, failure, cancellation, and `aclose()`; protocol-field-only response diagnostics with ordinary-error retention | Real streaming turn, upstream interruption, and client disconnect without duplication/truncation/stuck; Codex must receive the labeled failure instead of synthesizing unlabeled EOF; more than 4096 ordinary deltas must still complete; trace capacity may drop diagnostics but must not affect transport |
