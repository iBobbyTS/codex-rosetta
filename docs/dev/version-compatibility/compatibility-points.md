...
| Web search bridge | Codex `web_search` can be exposed to the Chat model, the `web_search_call` event can be reconstructed and continued after Tavily execution, and Tavily responses use the primary bounded identity-encoded HTTP reader. Hosted `web_search` resolves Provider/Token from the selected Tool Profile. Modified `web.run` instead resolves global `server.web_search`: Tavily uses its credential, while Self-hosted (Google) calls the authenticated existing `web-run` sidecar and normalizes result-page sources into the local string `output` contract; alpha.23 also carries optional structured SearchResponse `results` out-of-band. The configured-Responses mode selects exactly one enabled `responses` Provider plus one fixed Search Model (`gpt-5.6-sol` by default, `gpt-5.6-terra`, or `gpt-5.6-luna`), routes every non-Disabled Modified or Passthrough `web.run` SearchRequest to that Provider's relative `alpha/search`, and overwrites the final upstream `model` with the selected Search Model. Its transport credentials apply, while the current conversation route and upstream model alias cannot leak into the search request. The authenticated Admin Search Test still selects an eligible local source route to obtain the Tool Profile, builds one fixed Codex SearchRequest, and delegates to the same `handle_codex_auxiliary(..., "alpha/search")` entry point; the common auxiliary path applies the configured final model override. A shared five-second health cache coalesces bounded two-second sidecar probes for Admin and model requests; only online `browser_ready=true` adds self-hosted `search_query` plus model-visible browser commands, failures remove those commands without exposing connection details, and hot reload invalidates the cache. Ordinary Passthrough accepts Codex at `/v1/alpha/search` and appends relative `alpha/search` to the current Tool Mapping only upstream Base URL; neither forwarding path inserts `/v1` when its configured Base URL omits it | `gateway/web_search.py`, `gateway/codex_search.py`, `gateway/codex_auxiliary.py`, `gateway/config.py`, `gateway/admin/routes/config.py`, `gateway/admin/routes/network_search.py`, `gateway/web_run_sidecar.py`, `gateway/resources/web_run/google_search.py`, `gateway/transport/http/transport.py`, `test_web_search_bridge.py`, `test_codex_auxiliary.py`, `test_web_run_google_search.py`, `test_admin_config_routes.py`, `test_admin_network_search_routes.py`, `test_codex_source_contract.py`, WebUI network-search tests | native web-search item/event, tool configuration, global search configuration, Provider options, selected Responses Provider or Search Model validation/routing, Responses Lite model slugs, Admin diagnostic delegation, auxiliary response behavior, Google result-page behavior, upstream Base URL semantics, shared-cache lifecycle, or sidecar health semantics change |
...
| Web search bridge | Configure, disabled/missing keys, Search Model default/validation/persistence, exact three-option UI, ordinary and Admin final-body model override, search results, event reconstruction and continuation fixtures; real-loopback Content-Length/chunked/EOF/compression/timeout/cancel response limits | For configured Responses search, test all three reviewed models and verify that the selected value—not the conversation or local routing model—reaches the Provider's `/alpha/search`; then perform a real search, read results, continue to generate final answers, and verify that error paths are recoverable |
