...
| Deferred tool discovery | namespace defer, search matching, multiple searches, call/output, profile-route runtime discovery with static-profile eager ownership unchanged, two-way window isolation, UTF-8 byte/count budgets, per-principal unique-scope quota, same-principal global-oldest eviction, no cross-principal eviction, atomic overflow, concurrency, TTL/eviction/clear accounting | Run `tests/live_agent/deferred_tool_search` with a structured plugin mention; require real plugin/MCP search, call, consumed marker result, and verify that the two principals do not cross talk or exceed the retained-state envelope |
