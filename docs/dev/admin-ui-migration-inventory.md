...
| Provider configuration | `config`, provider PUT/DELETE/toggle, `/key`, `/models`; persisted `provider`, `registered_shims`, `known_api_types` | `ProvidersPage.svelte` | provider-and-URL-derived variants, backend-only API types, redirects, proxy, credential reveal and model discovery |
