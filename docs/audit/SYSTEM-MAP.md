...
| Provider transport | URL/auth header construction, canonical key rotation, client pool/SSE, credential return boundary | Project | Critical | `ProviderInfo`, `HttpTransport`, `CredentialRedactingTransport`, proxy passthrough/converted handlers | connection pools, key ring, actual wire credentials; credential-free output is preserved while exact return collisions fail closed, with raw SSE gated by complete events | vendored HTTP client | local process/container | `gateway/transport/`, `providers.py`, `gateway/proxy.py`, credential-return tests, AUD-017 closure |
...
| Optional web/search/image bridges | external search, web-run sidecar, image fetch/generation | Project | High | search/image routes and sidecar supervisor | health state, fetched content, tool references; reflected configured credentials produce controlled failures instead of rewritten content | Tavily/sidecar/provider endpoints | optional local/LAN | `gateway/web_run*`, `web_search.py`, `codex_images.py`, sidecar/auxiliary tests, AUD-017 closure |
...
| IF-03 | Gateway | upstream provider | OpenAI Chat/Responses, Anthropic Messages, Google GenAI HTTP | exact backend-supported or runtime-inferred `api_type`; URL-authoritative preset or custom endpoint | provider credential egress; arbitrary custom HTTP(S) egress accepted only within local/LAN scope; redirects denied by default and allowed only by explicit provider opt-in; exact return collisions fail closed without rewriting supported wire/application content | Gateway/provider owner + project owner | `config.py`, `providers.py`, `transport/`, `proxy.py`, converters, AUD-017 closure |
...
| Provider egress | Gateway process | converted request/model/config | provider API key in configured provider | prompt/tool payload and provider credential | request to configured preset or arbitrary custom HTTP(S) base URL/proxy; redirects fail closed by default, but explicit `allow_redirects` permits provider and model-discovery follow-ups; credential-free returns are preserved and exact collisions fail closed before unsafe bytes or objects reach consumers | operator configuration; custom egress and opt-in redirect expansion accepted for local/LAN only; arbitrary credential lengths remain supported | `GatewayConfig`, `ProviderInfo`, proxy, Admin discovery, redirect/redaction tests, profile, AUD-017 closure |
