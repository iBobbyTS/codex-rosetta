...
- Business goals: maintain Codex Responses/tool/stream/compaction semantics while routing to configured preset providers through a hub-and-spoke IR.
...
| Provider shims | Provider/model identity, defaults, transforms | Project | High | shim registry and `GatewayConfig` resolution | In-memory registry/config | bundled YAML/Python shims | Package resources | `shims/`, `tests/test_shims.py`, `tests/test_transforms.py` |
...
| Observability/persistence | request/error logs, metrics, mappings, retention and migration | Project | Critical | `PersistenceManager`, request/stream logging | SQLite WAL DB, mapping tables, trace JSONL | sqlite3; optional cryptography for mappings | local filesystem / `/config/data` | `observability/`, persistence tests |
...
| Optional web/search/image bridges | external search, web-run sidecar, image fetch/generation | Project | Medium | search/image routes and sidecar supervisor | health state, fetched content, tool references | Tavily/sidecar/provider endpoints | optional local/LAN | `gateway/web_run*`, `web_search.py`, `codex_images.py` |
...
| IF-03 | Gateway | upstream provider | OpenAI Chat/Responses, Anthropic Messages, Google GenAI HTTP | provider registry/shim and configured model group | provider credential egress; provider response untrusted | Gateway/provider owner | `providers.py`, `transport/`, converters |
...
| IF-06 | Gateway | SQLite persistence | request logs, error dumps, encrypted tool mappings, compaction mappings | current schema plus legacy migrations currently present | principal-scoped state; local filesystem | Observability owner | `observability/persistence.py`, tests |
...
| Codex compaction mappings | prompt/summary/source-sensitive | compaction rehydration | compaction response path | SQLite compaction table | rolling 7-day TTL; no explicit row/byte budget observed | no guarantee | `principal_id` + token hash | `codex_compaction.py`, persistence |
...
| Provider egress | Gateway process | converted request/model/config | provider API key in configured provider | prompt/tool payload and provider credential | outbound HTTP to configured base URL/proxy | operator configuration | `ProviderInfo`, transport |
...
| Agent audit execution | Codex/maintainer | repo files, test output, tool output | current task authorization | local repo/audit evidence; no real API secrets | read/write audit artifacts, local checks | no remediation authorization | `runs/20260719-1542/REPORT.md`, `../audit-profile.md` |
...
| `tests/live_agent` / agentabi | external model/CLI varies | real agent compatibility/eval | real credentials when developer-approved | network, provider, Codex CLI, browser/MCP fixtures | prompts, web/plugin/tool content | run roots, transcripts, outputs | explicit developer approval; excluded from audit | Developer/project owner |
...
| SCN-09 | Security/compatibility | Unsupported provider/base URL/custom path configured | registry/shim/config/admin UI | behavior matches approved preset-provider boundary or is explicitly rejected | config/admin tests + docs comparison | High | Gateway/product |
...
| Legacy/migration compatibility inventory has no approved removal boundary beyond user policy | stale paths may remain and expand maintenance/security surface | targeted inventory and owner-approved removal criteria | Open / Unknown |
