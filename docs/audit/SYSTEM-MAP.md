...
| Optional web/search/image bridges | external search, web-run sidecar, image fetch/generation | Project | High | search/image routes and sidecar supervisor | health state, fetched content, tool references; raw-before-parse collision checks share semantic JSON-escape and credential-domain gaps | Tavily/sidecar/provider endpoints | optional local/LAN | `gateway/web_run*`, `web_search.py`, `codex_images.py`, sidecar/auxiliary tests, AUD-019/AUD-020 |
...
| Provider egress | Gateway process | converted request/model/config | provider API key in configured provider | prompt/tool payload and provider credential | request to configured preset or arbitrary custom HTTP(S) base URL/proxy; redirects fail closed by default, but explicit `allow_redirects` permits provider and model-discovery follow-ups; raw equivalent JSON escapes can bypass collision checks and the authoritative configured-credential domain is pending | operator configuration; custom egress and opt-in redirect expansion accepted for local/LAN only; arbitrary credential lengths remain supported | `GatewayConfig`, `ProviderInfo`, proxy, Admin discovery, redirect/redaction tests, profile, AUD-019/AUD-020 |
...
| Return gates protect only the active provider credential inventory | another configured provider or client credential can be returned unchanged; global protection may also create unrelated short-token blocking | project owner must choose global runtime inventory or active-request-only scope before repair | Open / AUD-020; Needs Decision |
