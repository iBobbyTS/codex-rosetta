...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| GOV-01 | Profile, system boundary, owners, risk choices | Critical | baseline + material risk change | `20260720-1107` / 2026-07-20 / local | user decisions, provider semantics, and repository boundary | PROFILE + remediation report | no legal/contract obligations supplied | all areas | profile/deployment/risk change | Fresh | first deployment or scope change |
...
| PROVIDER-01 | URL-authoritative preset/custom provider resolution | Critical | every provider/Codex update + rotation | `20260720-1107` / 2026-07-20 / local | registry, shims, config resolution, Admin UI, explicit `api_type` validation | remediation evidence + provider/config/Admin tests | arbitrary custom HTTP(S) egress is owner-accepted local/LAN risk; no external call or provider-quality claim | SCN-03/09 | provider/shim/config/UI change | Fresh (deterministic) | provider policy or protocol change |
...
| DATA-01 | SQLite file permissions, schema, request/error retention | High | baseline + persistence change | `20260720-1107` / 2026-07-20 / local | SQLite init/schema fingerprint, logs, error dumps, redaction, retention | remediation evidence + persistence/retention tests | no restore/production disk exercise | SCN-07 | schema/retention/logging change | Fresh (schema contract) | first deployment or schema/retention change |
...
| AGENT-01 | Deterministic-vs-live test separation and approval | High | every harness/agent change | `20260720-1107` / 2026-07-20 / local | Makefile, live scripts, integration/agent launchers, runtime contract, shared gate | remediation evidence + full live-call contract tests | no live trajectory; approved runs can still cause real cost/side effects | SCN-11 | harness/credentials/agent permissions change | Fresh (deterministic) | new runner or harness permission change |
...
| SCN-09 | Unsupported custom/unknown provider config | exact preset URL renders preset; unmatched URL renders allowed `custom` | 2026-07-20 | Backend and Admin URL/protocol derivation, invalid `api_type` propagation, and profile tests pass | no egress/provider-quality test; custom egress risk accepted by owner | PROVIDER-01, SIDE-01 | provider/UI/config change | Fresh (deterministic) | provider policy or protocol change |
...
| SCN-11 | Agent/live runner external call | no audit calls; development call requires explicit approval | 2026-07-20 | every enumerated Python/shell entry point is gated and contract-tested | no live trajectory; approved live runs remain outside audit | AGENT-01 | harness/agent permission change | Fresh (deterministic) | new runner or approval-policy change |
...
| CTRL-06 | developer approval before real API tests | user decision/profile | shared gate plus complete integration/agent entry-point inventory | gate contract tests cover every enumerated entry point and exact-marker fail-closed behavior | no live trajectory by policy | Test harness owner | Fresh (deterministic) |
