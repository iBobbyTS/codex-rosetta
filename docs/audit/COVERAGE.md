...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| GOV-01 | Profile, system boundary, owners, risk choices | Critical | baseline + material risk change | `804efef` + `20260720-1239` / 2026-07-20 / local | user decisions, provider semantics, redirect boundary, and repository boundary | PROFILE + remediation report | no legal/contract obligations supplied | all areas | profile/deployment/risk change | Fresh | first deployment or scope change |
...
| PROVIDER-01 | URL-authoritative preset/custom provider resolution and egress | Critical | every provider/Codex update + rotation | `804efef` / 2026-07-20 / local | registry, shims, config inference, Admin rendering, no write-back, redirect rejection | `20260720-1239` evidence + provider/config/Admin/transport tests | direct arbitrary custom HTTP(S) egress is owner-accepted local/LAN risk; no external call or provider-quality claim | SCN-03/09 | provider/shim/config/UI/transport change | Fresh (deterministic) | provider policy or protocol change |
...
| AGENT-01 | Deterministic-vs-live test separation and approval | High | every harness/agent change | `35521ab` / 2026-07-20 / local | Makefile, all REST/SDK examples, live scripts, integration/agent launchers, runtime contract, shared gate | `20260720-1239` evidence + directory-complete live-call contract tests | no live trajectory; approved runs can still cause real cost/side effects | SCN-11 | harness/example/credentials/agent permissions change | Fresh (deterministic) | new runner or harness permission change |
...
| SCN-03 | Codex Responses route to configured provider | preserve required request/response semantics; do not follow redirects | 2026-07-20 | converters/proxy + source contract + loopback redirect rejection; deterministic satisfied | real upstream/model unknown | PROVIDER-01, CODEX-01 | route/converter/transport/Codex change | Fresh (deterministic) | live approved development run |
...
| SCN-09 | Missing-protocol or custom provider config | exact preset URL renders preset and first supported protocol; unmatched URL renders allowed `custom` and defaults to Responses; inference warns and is not persisted; redirects fail closed | 2026-07-20 | Backend/Admin inference, warning/no-write-back, profile, and loopback redirect tests pass | no real egress/provider-quality test; direct custom egress risk accepted by owner | PROVIDER-01, SIDE-01 | provider/UI/config/transport change | Fresh (deterministic) | provider policy or protocol change |
...
| SCN-11 | Agent/live runner external call | no audit calls; development call requires explicit approval | 2026-07-20 | every integration/agent launcher and all 24 directory-discovered examples gate before sensitive work | no live trajectory; approved live runs remain outside audit | AGENT-01 | harness/example/agent permission change | Fresh (deterministic) | new runner or approval-policy change |
...
| CTRL-06 | developer approval before real API tests | user decision/profile | shared gate plus complete integration/agent/example entry-point inventory | gate contract tests discover both example directories and verify ordering before dotenv | no live trajectory by policy | Test harness owner | Fresh (deterministic) |
...
| CTRL-09 | upstream authorization never follows redirects | owner decision/profile | shared HTTP client pool with zero redirects | independent loopback target remains untouched for redirect responses | direct configured URL/proxy egress still accepted | Gateway transport owner | Fresh (deterministic) |
