...
def project_modified_web_run_function(
    ...
    if not isinstance(parameters, dict):
        return None
    projected_parameters = project_modified_web_run_schema(parameters)
    ...
    if isinstance(description, str):
        projected["description"] = project_modified_web_run_description(description)
    return projected
...
def project_modified_web_run_schema(
    ...
    for command, allowed_fields in WEB_RUN_SUPPORTED_COMMAND_FIELDS.items():
...
def project_modified_web_run_description(description: str) -> str:
    """Remove unsupported command guidance from the live Codex description."""
    retained: list[str] = []
    unsupported_markers = tuple(
        marker
        for command in sorted(WEB_RUN_UNSUPPORTED_COMMANDS)
