    ...
    async def execute(
        ...
        if isinstance(candidate, TavilySearchProviderCandidate):
            ...
            if self._responses_client is not None:
                return _validate_response(await self._responses_client(candidate, body))
            return _validate_response(await self._call_responses(candidate, body))
        if client is None:
        ...
        for query, settings in snapshot.queries:
            try:
                raw = await client.search(query, settings=settings)
