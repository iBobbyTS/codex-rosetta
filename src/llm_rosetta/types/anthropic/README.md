...
This directory contains TypedDict replicas of Anthropic SDK types for use in LLM-Rosetta's conversion layer.

## Overview

These types are **replicas** of the original Anthropic SDK types, converted from Pydantic `BaseModel` to `TypedDict` for better compatibility with LLM-Rosetta's type system and to avoid runtime dependencies on the Anthropic SDK.
...
- `/data/pding/miniforge3/envs/llm_rosetta/lib/python3.10/site-packages/anthropic/types/`
