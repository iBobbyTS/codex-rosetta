...
LLMIR 项目正在将所有 converter 从旧的单文件架构重构为 Bottom-Up Ops Pattern。
...
- 删除 `src/llmir/utils/` 目录和 `tests/utils/` 目录
- 删除 `src/llmir/types/providers/` 空目录
- 更新 `plans/architecture.md`（移除 DEPRECATED 标记，更新重构状态表）
- 更新 `src/llmir/converters/__init__.py`（恢复导出所有 converter）
- 更新 `src/llmir/__init__.py`（恢复导出所有 converter）

## 关键参考文件

- **架构设计**：`plans/architecture.md`
- **Base ABC**：`src/llmir/converters/base/`
- **OpenAI Chat 参考实现**：`src/llmir/converters/openai_chat/`
- **Anthropic 参考实现**：`src/llmir/converters/anthropic/`
- **IR 类型**：`src/llmir/types/ir/`
- **流式事件类型**：`src/llmir/types/ir/stream.py`