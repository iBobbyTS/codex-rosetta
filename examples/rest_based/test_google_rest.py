#!/usr/bin/env python
"""
REST API test for Google Gemini API
...
def main():
    print("=" * 80)
    print("Google Gemini API REST Test")
    print("=" * 80)
    ...
    print("\n--- Converting IR to Google format ---")
    ...
    print(f"Contents: {len(payload['contents'])}")
    ...
    if "tool_config" in payload:
        request_body["tool_config"] = payload["tool_config"]
    ...
    try:
        ...
        if ir_from_response and len(ir_from_response) > 0:
            ...
            if tool_calls:
                ...
                print(f"\nFinal payload contents: {len(final_payload['contents'])}")

                final_request_body = {
                    "contents": final_payload["contents"],
                    "tools": final_payload.get("tools"),
                }

                if "tool_config" in final_payload:
                    final_request_body["tool_config"] = final_payload["tool_config"]
