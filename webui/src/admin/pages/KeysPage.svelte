...
<Modal open={createOpen} labelledby="key-modal-title" onclose={()=>createOpen=false}>{#snippet header()}<h3 id="key-modal-title">{t('modal.generateKey','Generate API Key')}</h3>{/snippet}<div class="form-group"><label for="keyLabel">{t('label.keyLabel','Label (optional)')}</label><input id="keyLabel" maxlength="128" bind:value={label} placeholder="e.g. Production, Dev testing" /></div><div class="form-group"><label for="keyManual">{t('label.keyManual','Manual key (leave empty to auto-generate)')}</label><input id="keyManual" bind:value={manualKey} autocomplete="new-password" placeholder="rsk-... (auto-generated if empty)" style="font-family:var(--mono)" /></div>{#snippet actions()}<button class="btn" onclick={()=>createOpen=false}>{t('btn.cancel','Cancel')}</button><button class="btn btn-primary" disabled={busy} onclick={()=>void create()}>{t('btn.generate','Generate')}</button>{/snippet}</Modal>
<Modal open={createdOpen} labelledby="key-created-title" onclose={()=>createdOpen=false}>{#snippet header()}<h3 id="key-created-title">{t('modal.keyCreated','Key Created')}</h3>{/snippet}<p style="font-size:13px;color:var(--text-dim);margin-bottom:12px">{t('keys.copyWarning','Copy this key now. It will not be shown again in full.')}</p><div style="display:flex;gap:8px;align-items:center"><input readonly value={createdKey} style="flex:1;padding:8px 12px;background:var(--bg);border:1px solid var(--border);border-radius:var(--radius);color:var(--text);font-size:14px;font-family:var(--mono)" /><button class="btn btn-sm" onclick={()=>void copy()}>{t('btn.copy','Copy')}</button></div>{#snippet actions()}<button class="btn btn-primary" onclick={()=>createdOpen=false}>{t('btn.close','Close')}</button>{/snippet}</Modal>
