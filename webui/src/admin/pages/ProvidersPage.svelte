<script lang="ts">
  ...
  let name = $state(''); let urlRows = $state<UrlRow[]>([initialUrlRow]); let currentUrlRowId = $state(initialUrlRow.id); let proxy = $state(''); let apiType = $state(''); let requestEncoding = $state<RequestEncoding>('passthrough'); let allowRedirects = $state(false); let softInterrupt = $state(false); let forceRosettaCompaction = $state(false);
  ...
  function clearForm(): void {
    const url=urlRow('');const credential=credentialRow({id:'primary',key:''});editingName=''; name=''; urlRows=[url]; currentUrlRowId=url.id; proxy=''; apiType=allowedTypes()[0] ?? ''; requestEncoding='passthrough'; allowRedirects=false; softInterrupt=false; forceRosettaCompaction=false; vendorId='custom'; variantId='custom'; credentialRows=[credential];currentCredentialRowId=credential.rowId; error='';
  }
...
<Modal open={modalOpen} className="provider-modal" labelledby="provider-modal-title" onclose={()=>modalOpen=false}>
  ...
  {#if apiType==='responses'}<div class="form-group"><label for="provRequestEncoding">{t('label.requestEncoding')}</label><Dropdown id="provRequestEncoding" value={requestEncoding} options={['passthrough','identity','zstd'].map((value)=>({value,label:t(`provider.requestEncoding.${value}`)}))} fitViewport={true} fitContent={true} menuAlign="left" onChange={(value:DropdownValue)=>{requestEncoding=String(value) as RequestEncoding;}} /><p class="provider-option-description">{t('provider.requestEncodingDescription')}</p></div>{/if}
