<script lang="ts">
  ...
  let pendingDelete = $state(''); let deleteInput = $state(''); let credentialDeleteGroups = $state<string[]>([]); let pendingProviderBody = $state<Record<string,unknown>|null>(null); let detectionGeneration = 0;
  ...
  function closeProviderModal(): void { invalidateDetection();modalOpen=false;credentialDeleteGroups=[];pendingProviderBody=null; }
  ...
  async function submitProvider(body:Record<string,unknown>,confirmed=false):Promise<void>{busy=true;error='';try{await api.put(`/admin/api/config/providers/${encodeURIComponent(name.trim())}`,confirmed?{...body,confirm_credential_deletion:true}:body);credentialDeleteGroups=[];pendingProviderBody=null;closeProviderModal();notice=t('toast.providerSaved',{name:name.trim()});await load();}catch(cause){const groups=referencedCredentialGroups(cause);if(groups&&!confirmed){credentialDeleteGroups=groups;pendingProviderBody=body;}else error=message(cause);}finally{busy=false;}}
...
<ConfirmDialog open={credentialDeleteGroups.length>0} title={t('confirm.deleteReferencedCredentialsTitle')} message={t('confirm.deleteReferencedCredentials',{groups:credentialDeleteGroups.join(', ')})} confirmLabel={t('btn.delete')} cancelLabel={t('btn.cancel')} closeLabel={t('btn.cancel')} intent="danger" confirmDisabled={busy} cancelDisabled={busy} onClose={()=>{credentialDeleteGroups=[];pendingProviderBody=null;}} onConfirm={()=>{if(pendingProviderBody)void submitProvider(pendingProviderBody,true);}} />
