<script lang="ts">
  ...
  let accounts = $state<Account[]>([]); let accountsError = $state(''); let boundAccountId = $state(''); let boundItems = $state<BoundKeyItem[]>([]); let bindingBusy = $state(false); let bindingError = $state(''); let bindingGeneration = 0; let bindingController:AbortController|null=null;
  ...
  function invalidateBinding():void{bindingGeneration+=1;bindingController?.abort();bindingController=null;bindingBusy=false;bindingError='';}
  async function fetchBoundItems(accountId:string,trigger:'open'|'select'):Promise<void>{const generation=++bindingGeneration;bindingController?.abort();const controller=new AbortController();bindingController=controller;const providerName=editingName;bindingBusy=true;bindingError='';const isCurrent=():boolean=>modalOpen&&generation===bindingGeneration&&providerName===editingName;try{const result=await api.post<unknown>(bindingPath(providerName),{account_id:accountId},controller.signal);if(!isCurrent())return;const items=parseBoundItems(result);boundItems=items;if(trigger==='select'){boundAccountId=accountId;credentialRows=[];currentCredentialRowId='';}}catch(cause){if(isCurrent()&&!(cause instanceof DOMException&&cause.name==='AbortError'))bindingError=t('error.providerBindingFetch',{error:message(cause)});}finally{if(isCurrent()){bindingBusy=false;bindingController=null;}}}
  function selectBinding(value:string):void{if(value===''){invalidateBinding();boundAccountId='';boundItems=[];return;}if(value===boundAccountId)return;void fetchBoundItems(value,'select');}
  ...
  async function load(signal?: AbortSignal): Promise<void> { const [configResult,accountsResult]=await Promise.allSettled([api.get<Config>('/admin/api/config',signal),api.get<{accounts?:Account[]}>('/admin/api/accounts',signal)]);if(configResult.status==='fulfilled'){config=configResult.value;error='';}else if(!(configResult.reason instanceof DOMException&&configResult.reason.name==='AbortError'))error=message(configResult.reason);if(accountsResult.status==='fulfilled'){accounts=(accountsResult.value.accounts??[]).filter((account)=>account.provider==='sub2api');accountsError='';}else if(!(accountsResult.reason instanceof DOMException&&accountsResult.reason.name==='AbortError')){accounts=[];accountsError=t('error.providerAccountsFetch',{error:message(accountsResult.reason)});}loading=false; }
  ...
  async function save(): Promise<void> {
    const body=providerBody();if(body)await submitProvider(body);
  }
...
<Modal open={modalOpen} className="provider-modal" labelledby="provider-modal-title" onclose={closeProviderModal}>
  ...
  {#snippet actions()}<button class="btn" onclick={closeProviderModal}>{t('btn.cancel')}</button><button class="btn btn-primary" disabled={busy} onclick={()=>void save()}>{t('btn.save')}</button>{/snippet}
</Modal>
