<script lang="ts">
  ...
  let pendingDelete = $state(''); let deleteInput = $state(''); let detectionGeneration = 0;
  ...
  function closeProviderModal(): void { invalidateDetection();modalOpen=false; }
  ...
  function clearForm(): void {
    invalidateDetection();
    const url=urlRow('');const credential=credentialRow({uuid:crypto.randomUUID(),id:'primary',key:''});editingName=''; name=''; urlRows=[url]; currentUrlRowId=url.id; proxy=''; apiType=allowedTypes()[0] ?? ''; requestEncoding='passthrough'; detectionModel=''; detectingEncoding=false; detectionError=''; allowRedirects=false; softInterrupt=false; forceRosettaCompaction=false; vendorId='custom'; variantId='custom'; credentialRows=[credential];currentCredentialRowId=credential.rowId; error='';
  }
  ...
  async function save(): Promise<void> {
    ...
    busy=true; error=''; try { const body: Record<string,unknown>={provider:vendorId,api_type:apiType,base_urls:normalizedUrls,current_base_url:currentUrl,api_keys:normalizedCredentials,current_api_key:currentCredential,proxy:proxy.trim(),allow_redirects:allowRedirects}; if(apiType==='chat')body.soft_interrupt=softInterrupt; if(apiType==='responses'){body.request_encoding=requestEncoding;body.force_rosetta_compaction=forceRosettaCompaction;} if(editingName&&editingName!==name.trim())body.rename_from=editingName; await api.put(`/admin/api/config/providers/${encodeURIComponent(name.trim())}`,body); closeProviderModal(); notice=t('toast.providerSaved',{name:name.trim()}); await load(); } catch(cause){error=message(cause);} finally{busy=false;}
  }
