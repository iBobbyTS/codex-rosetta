<script lang="ts">
  ...
  type Provider = { provider?: string; base_urls?: string[]; current_base_url?: string; base_url_statuses?: BaseUrlStatus[]; api_type?: string; proxy?: string; enabled?: boolean; allow_redirects?: boolean; soft_interrupt?: boolean; force_rosetta_compaction?: boolean; api_key?: string; validation_error?: string };
  ...
  function clearForm(): void {
    editingName=''; name=''; urls=['']; currentUrl=''; proxy=''; apiType=allowedTypes()[0] ?? ''; allowRedirects=false; softInterrupt=false; forceRosettaCompaction=false; vendorId='custom'; variantId='custom'; apiKey=''; keyVisible=false; error='';
  }
  ...
  async function save(): Promise<void> {
    ...
    busy=true; error=''; try { const body: Record<string,unknown>={provider:vendorId,api_type:apiType,base_urls:normalizedUrls,current_base_url:normalizeUrl(currentUrl),proxy:proxy.trim(),allow_redirects:allowRedirects}; if(apiType==='chat')body.soft_interrupt=softInterrupt; if(apiType==='responses')body.force_rosetta_compaction=forceRosettaCompaction; if(credential)body.api_key=credential; if(editingName&&editingName!==name.trim())body.rename_from=editingName; await api.put(`/admin/api/config/providers/${encodeURIComponent(name.trim())}`,body); modalOpen=false; notice=t('toast.providerSaved',{name:name.trim()}); await load(); } catch(cause){error=message(cause);} finally{busy=false;}
  }
    ...
    <div class="provider-grid" class:list-view={view==='list'}>
      {#each filteredEntries as [providerName, provider]}
        ...
        <div class="provider-card" class:disabled={!enabled} class:config-error={!!provider.validation_error}>
          ...
          {#if view==='list'}
            <div class="field" title={`${info.vendor} · ${info.protocol}`}><code>{info.vendor} · {info.protocol}</code></div><div class="field" title={firstUrl(provider)}><code>{firstUrl(provider)}</code></div><div class="field"><code>{config.credential_visible===false?'':provider.api_key ?? ''}</code></div>
          {:else}
            <div class="field">{t('label.providerVendor')}: <code>{info.vendor}</code></div><div class="field">{t('label.providerProtocol')}: <code>{info.protocol}</code></div>{#each provider.base_url_statuses??[] as item}<div class="provider-url-status" class:available={item.status==='available'} class:cooling={item.status==='cooling'}><code>{item.base_url}</code><span>{t(`provider.url.${item.status}`)}</span>{#if item.current}<span>{t('provider.url.current')}</span>{:else}<button class="btn btn-sm" aria-label={t('aria.makeBaseUrlCurrent',{url:item.base_url})} onclick={()=>void selectCurrent(providerName,item.base_url)}>{t('provider.url.select')}</button>{/if}</div>{/each}{#if config.credential_visible!==false}<div class="field">{t('label.apiKeyShort')} <code>{provider.api_key ?? ''}</code></div>{/if}{#if provider.proxy}<div class="field">{t('label.proxyShort')} <code>{provider.proxy}</code></div>{/if}
          {/if}
...
<Modal open={modalOpen} labelledby="provider-modal-title" onclose={()=>modalOpen=false}>
  ...
  <div class="form-group"><label for="provApiType">{t('label.providerProtocol')}</label><Dropdown id="provApiType" value={apiType} optionGroups={protocolGroups().map((group)=>({label:group.label,options:group.items.map((item)=>({value:item,label:protocolLabel(item)}))}))} fitViewport={true} onChange={(value:DropdownValue)=>chooseProtocol(String(value))} /></div>
  <div class="form-group"><div class="form-label">{t('label.baseUrls')}<span class="hint-icon">?<span class="hint-popup">{t('hint.docker')}</span></span></div>{#each urls as item,index}<div class="provider-url-editor"><input aria-label={t('aria.baseUrl',{index:index+1})} value={item} oninput={(event)=>setUrl(index,event.currentTarget.value)} placeholder={t('placeholder.baseUrl')} /><label><input type="radio" name="current-base-url" checked={currentUrl===item} onchange={()=>currentUrl=item} />{t('provider.url.current')}</label><button type="button" class="btn btn-sm" disabled={index===0} aria-label={t('aria.moveBaseUrlUp',{url:item})} onclick={()=>moveUrl(index,-1)}>↑</button><button type="button" class="btn btn-sm" disabled={index===urls.length-1} aria-label={t('aria.moveBaseUrlDown',{url:item})} onclick={()=>moveUrl(index,1)}>↓</button><button type="button" class="btn btn-sm" disabled={urls.length===1} aria-label={t('aria.removeBaseUrl',{url:item})} onclick={()=>removeUrl(index)}>{t('btn.remove')}</button></div>{/each}<button type="button" class="btn btn-sm" onclick={addUrl}>{t('btn.addBaseUrl')}</button></div>
...
<style>.provider-url-status,.provider-url-editor{display:flex;align-items:center;gap:6px;margin:5px 0}.provider-url-status{padding:5px 7px;border-radius:var(--radius);font-size:11px}.provider-url-status.available{background:color-mix(in srgb,var(--green) 10%,transparent)}.provider-url-status.cooling{background:color-mix(in srgb,var(--orange) 12%,transparent)}.provider-url-status code,.provider-url-editor input{min-width:0;flex:1}.provider-url-editor label{display:flex;gap:4px;align-items:center;white-space:nowrap;font-size:11px}.provider-url-editor label input{width:auto}</style>
