<script lang="ts">
  ...
  async function save(): Promise<void> {
    ...
    busy=true; error=''; try { const body: Record<string,unknown>={provider:vendorId,api_type:apiType,base_url:url.trim(),proxy:proxy.trim(),allow_redirects:allowRedirects}; if(credential)body.api_key=credential; if(editingName&&editingName!==name.trim())body.rename_from=editingName; await api.put(`/admin/api/config/providers/${encodeURIComponent(name.trim())}`,body); modalOpen=false; notice=t('toast.providerSaved',"Provider '{name}' saved").replace('{name}',name.trim()); await load(); } catch(cause){error=message(cause);} finally{busy=false;}
  }
...
<Modal open={modalOpen} labelledby="provider-modal-title" onclose={()=>modalOpen=false}>
  {#snippet header()}<h3 id="provider-modal-title">{editingName?t('modal.editProvider','Edit Provider'):t('modal.addProvider','Add Provider')}</h3>{/snippet}
  ...
  {#snippet actions()}<button class="btn" onclick={()=>modalOpen=false}>{t('btn.cancel','Cancel')}</button><button class="btn btn-primary" disabled={busy} onclick={()=>void save()}>{t('btn.save','Save')}</button>{/snippet}
</Modal>

<Modal open={deleteOpen} labelledby="delete-provider-title" onclose={()=>deleteOpen=false}>
  {#snippet header()}<h3 id="delete-provider-title">{t('confirm.deleteProviderTitle','Delete Provider')}</h3>{/snippet}
  <p style="margin:8px 0 12px;color:var(--text-dim);font-size:13px">Type <strong>{pendingDelete}</strong> to confirm.</p>
  ...
  <div class="form-group"><input aria-label="Confirm provider name" bind:value={deleteInput} autocomplete="off" spellcheck="false" style="font-family:monospace" /></div>
  {#snippet actions()}<button class="btn" onclick={()=>deleteOpen=false}>{t('btn.cancel','Cancel')}</button><button class="btn btn-danger" disabled={deleteInput!==pendingDelete||busy} onclick={()=>void confirmDelete()}>{t('btn.delete','Delete')}</button>{/snippet}
</Modal>
