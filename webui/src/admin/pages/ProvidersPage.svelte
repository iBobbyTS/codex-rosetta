<script lang="ts">
  ...
  function providerBody():Record<string,unknown>|null{
    ...
    const body:Record<string,unknown>={provider:vendorId,api_type:apiType,base_urls:normalizedUrls,current_base_url:currentUrl,api_keys:normalizedCredentials,current_api_key:currentCredential,auto_rotate_credentials:autoRotateCredentials,proxy:proxy.trim(),allow_redirects:allowRedirects};if(boundAccountId)body.sub2api_account_id=boundAccountId;if(apiType==='chat')body.soft_interrupt=softInterrupt;if(apiType==='responses'){body.request_encoding=requestEncoding;body.force_rosetta_compaction=forceRosettaCompaction;}if(editingName&&editingName!==name.trim())body.rename_from=editingName;return body;
  }
